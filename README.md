...
* **Kalpa** (`include/kalpa/`) — Header-only, concept-based mathematical optimization library and a thin orchestration layer over the numeric stack: it owns the iteration loop, policy plumbing, and diagnosis while delegating linear algebra to `ga`, autodiff/HVP to `ga::Dual`, parallel evaluation to `pravaha`, telemetry to `nadi`, caching to `kosha`, and the EDSL graph to `vakya`.
  ...
  * Unconstrained: GD, Momentum, Adam, nonlinear CG, L-BFGS, BFGS, Newton, matrix-free trust-region Newton-CG (`kalpa/algo/unconstrained.hpp`).
  * Constrained: native n-D box/ball/polytope projection, projected-gradient, Frank–Wolfe, equality-QP, dense LP, and exact-rational LP on native `Fraction` arrays (`kalpa/algo/constrained.hpp`).
  * Global / derivative-free: Nelder–Mead, differential evolution, simulated annealing, CMA-ES, with seedable deterministic RNG and serial/`pravaha`-parallel population evaluation (`kalpa/algo/global.hpp`).
  * Fluent/EDSL front end over `vakya` (`minimize(sq(x[0]-constant(1))+…)`); `nadi`-backed introspection (`FullTrace`, `ProgressBar`, `Callback`) with a zero-overhead `NoTelemetry` default (`kalpa/introspect/telemetry.hpp`).
