        ...
        for (int sub = 0; sub < substeps_; ++sub) {
            ...
            for (const auto& [a, b] : pairs) {
                ...
                if (cached.has_value() &&
                    ...
                    const akruti::TransformedShape tsA{bA.shape, akruti::Vec{bA.position[0], bA.position[1]}, bA.angle};
                    const akruti::TransformedShape tsB{bB.shape, akruti::Vec{bB.position[0], bB.position[1]}, bB.angle};
        ...
        for (auto& b : bodies_) {
            ...
            if (pebble::math::length_sq(motion) > 1.0f) {
                ...
                for (const auto& other : bodies_) {
                    ...
                    const akruti::TransformedShape tsB{other.shape, akruti::Vec{other.position[0], other.position[1]}, other.angle};
