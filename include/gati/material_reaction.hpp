    ...
    struct MaterialReactionSystem {
        ...
        static void evaluate_reactions(pebble::ecs::World& world, const ContactEvent& ce,
            ...
            if (mat_a && mat_a->phase_fractions.solid() > 0.5f) {
                ...
                const float sound_speed = std::sqrt(std::max(
                    1.0f, (mat_a->params.youngs_modulus > 0 ? mat_a->params.youngs_modulus : 1e7f) / density));
            ...
            if (mat_b && world.alive(ent_b) && mat_b->phase_fractions.solid() > 0.5f) {
                ...
                const float sound_speed = std::sqrt(std::max(
                    1.0f, (mat_b->params.youngs_modulus > 0 ? mat_b->params.youngs_modulus : 1e7f) / density));
                const float strain = impact_speed / sound_speed;
            ...
            if (mat_a && mat_b && world.alive(ent_a) && world.alive(ent_b)) {
                ...
                const float k_eff = (mat_a->params.conductivity + mat_b->params.conductivity) * 0.5f *
                ...
                if (mat_a->can_fuse && mat_b->can_fuse) {
                    const bool a_molten = (mat_a->phase_fractions.liquid() > 0.5f || mat_a->phase_fractions.plastic() >
                        0.5f);
                    const bool b_molten = (mat_b->phase_fractions.liquid() > 0.5f || mat_b->phase_fractions.plastic() >
                        0.5f);
