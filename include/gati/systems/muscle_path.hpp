...
struct PathUpdateSystem {
    ...
    void run(World& world, StepContext ctx) {
        ...
        world.par_view<MuscleController, MusclePath>(ctx.executor, [this](Entity, MuscleController& ctrl, MusclePath& path) {
            ...
            for (std::size_t i = 1; i < path.via_points.size(); ++i) {
                const Vec2 d = path.via_points[i].position - path.via_points[i - 1].position;
                len += pebble::math::length(d);
            }
            path.cached_path_length = len;
            muscle_store->rest_length[ctrl.prakriti_constraint_id] = std::max(len, 1e-4f);
        });
    }
};
#endif
