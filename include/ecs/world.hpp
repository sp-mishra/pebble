    ...
    public:
        ...
        void view(Fn&& fn) {
            ...
            if constexpr (sizeof...(Rest) == 0) {
                ...
                ((try_store<Rest>()->size() < min_size
                      ? (min_size = try_store<Rest>()->size(), min_idx = curr_idx, 0)
                      : (++curr_idx, 0)), ...);
        ...
        template <typename WithClause, typename WithoutClause = Without<>, typename OptionalClause = Optional<void>,
                  typename Fn>
        void query(Fn&& fn) {
