...
namespace pebble::drishya {
        ...
        void replay(P& painter) const {
            ...
            for (DrawOp op : op_) {
                ...
                case DrawOp::SetColor:
                    ...
                    ++iu;
                    break;
                case DrawOp::PushClip: painter.push_clip(bounds_[ib++]);
                    break;
                case DrawOp::PopClip: painter.pop_clip();
                    break;
                case DrawOp::FillRect: painter.fill_rect(rect_[ir].r);
                    ++ir;
                    break;
                case DrawOp::StrokeRect: painter.stroke_rect(rect_[ir].r, rect_[ir].aux);
                    ++ir;
                    break;
                case DrawOp::RoundRect: painter.round_rect(rect_[ir].r, rect_[ir].aux);
                    ++ir;
                    break;
                case DrawOp::Line: painter.line(line_[il].a, line_[il].b, line_[il].width);
                    ++il;
                    break;
                case DrawOp::Text: painter.text(std::string_view{text_[it].s}, text_[it].pos, text_[it].font_size);
                    ++it;
                    break;
                }
            }
    ...
    private:
        struct RectCmd {
            Rect2D r;
            float aux;
        }; // aux = stroke width or corner radius
        struct Line {
            Vec2 a, b;
            float width;
        };
