...
public:
    ...
    void fit(const ts::tensor<float>& X) {
        ...
        for (int iter = 0; iter < max_iter_; ++iter) {
            ...
            for (size_t i = 0; i < n; ++i) {
                ...
                if (labels_[i] != best) { labels_[i] = best; changed = true; }
            }
            // Update step
    ...
    ts::tensor<int> predict(const ts::tensor<float>& X) const {
        ...
        ts::tensor<int> out({n});
        for (size_t i = 0; i < n; ++i) {
            int best = 0; float best_d = std::numeric_limits<float>::max();
            for (int c = 0; c < k_; ++c) {
                float d = sq_dist(X, i, f, c);
                if (d < best_d) { best_d = d; best = c; }
            }
...
private:
    int k_, max_iter_; float tol_; uint64_t seed_;
    bool fitted_ = false;
            ...
            for (int ci = 1; ci < k_; ++ci) {
                ...
                for (size_t i = 0; i < n; ++i) {
                    ...
                    for (int cj = 0; cj < ci; ++cj) {
                        float d = 0.0f;
                        for (size_t j = 0; j < f; ++j) { float dv = X({i,j}) - c({static_cast<size_t>(cj),j}); d += dv*dv; }
    ...
    float sq_dist(const ts::tensor<float>& X, size_t i, size_t f, int c) const {
        float d = 0.0f;
        for (size_t j = 0; j < f; ++j) { float v = X({i,j}) - centers_({static_cast<size_t>(c),j}); d += v*v; }
        return d;
    }
