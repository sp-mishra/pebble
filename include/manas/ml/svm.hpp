        ...
        for (int iter = 0; iter < max_iter_; ++iter) {
            ...
            for (size_t i = 0; i < n; ++i) {
                ...
                if ((yi * Ei < -tol_ && alphas_[i] < C_) ||
                    ...
                    float L, H;
                    if (yi != yj) { L = std::max(0.0f, aj_old - ai_old); H = std::min(C_, C_ + aj_old - ai_old); }
                    else          { L = std::max(0.0f, ai_old + aj_old - C_); H = std::min(C_, ai_old + aj_old); }
                    if (L >= H) continue;
                    ...
                    float b1 = b_ - Ei - yi*(alphas_[i]-ai_old)*K[i][i] - yj*(alphas_[j]-aj_old)*K[i][j];
                    float b2 = b_ - Ej - yi*(alphas_[i]-ai_old)*K[i][j] - yj*(alphas_[j]-aj_old)*K[j][j];
                    b_ = (alphas_[i] > 0.0f && alphas_[i] < C_) ? b1 :
                         (alphas_[j] > 0.0f && alphas_[j] < C_) ? b2 : 0.5f*(b1+b2);
                    ++changed;
                }
            }
            if (changed == 0) break;
