...
struct SqrtFeatures { static size_t count(size_t f) { return std::max(size_t{1}, static_cast<size_t>(std::sqrt(static_cast<float>(f)))); } };
struct LogFeatures  { static size_t count(size_t f) { return std::max(size_t{1}, static_cast<size_t>(std::log2(static_cast<float>(f)) + 1)); } };
struct AllFeatures  { static size_t count(size_t f) { return f; } };
...
public:
    ...
    ts::tensor<float> predict(const ts::tensor<float>& X) const {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            float best_lbl = 0.0f; int best_cnt = 0;
            for (auto& [lbl, cnt] : votes) if (cnt > best_cnt) { best_cnt = cnt; best_lbl = lbl; }
            out({i}) = best_lbl;
        }
        return out;
    }
