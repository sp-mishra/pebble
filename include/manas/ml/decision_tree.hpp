...
inline float gini(std::span<const float> labels, size_t n) {
    ...
    float g = 1.0f;
    for (auto& [k, c] : counts) { float p = static_cast<float>(c) / n; g -= p * p; }
    return g;
}
...
inline float entropy(std::span<const float> labels, size_t n) {
    ...
    float e = 0.0f;
    for (auto& [k, c] : counts) {
        if (c > 0) { float p = static_cast<float>(c) / n; e -= p * std::log2(p); }
    }
    return e;
}
...
inline float mse(std::span<const float> labels, size_t n) {
    ...
    float s = 0.0f;
    for (float v : labels) { float d = v - mean; s += d * d; }
...
inline float majority_class(const std::vector<float>& labels) {
    std::unordered_map<float, int> counts;
    for (float v : labels) counts[v]++;
    float best = labels[0]; int best_cnt = 0;
    for (auto& [k, c] : counts) if (c > best_cnt) { best_cnt = c; best = k; }
    return best;
}
...
private:
    ...
    std::unique_ptr<detail::Node> build(const ts::tensor<float>& X,
        ...
        int best_feat = -1; float best_thresh = 0.0f;
        ...
        for (size_t f = 0; f < n_features_; ++f) {
            ...
            for (size_t vi = 0; vi + 1 < vals.size(); ++vi) {
                ...
                if (static_cast<int>(left_labels.size()) < min_samples_leaf_ ||
                    static_cast<int>(right_labels.size()) < min_samples_leaf_) continue;
                ...
                if (gain > best_gain) { best_gain = gain; best_feat = static_cast<int>(f); best_thresh = thresh; }
            }
        }

        if (best_feat == -1) {
            node->is_leaf = true; node->value = leaf_value(labels); return node;
        }
