...
using SelectionPolicy = std::function<std::vector<BrainGenome>(const std::vector<BrainGenome>&, const std::vector<FitnessMetrics>&)>;
...
struct TournamentSelection {
    ...
    std::vector<BrainGenome> operator()(const std::vector<BrainGenome>& population, const std::vector<FitnessMetrics>& scores) const {
