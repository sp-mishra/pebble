...
private:
    float lr_, momentum_, wd_; bool nesterov_; ClipPolicy clip_;
...
public:
    ...
    void step(const ParamList& params) {
        ...
        for (auto* p : params) {
            ...
            auto& m = m_[p];  auto& v = v_[p];
            if (m.size() != n) { m.assign(n, 0.0f); v.assign(n, 0.0f); }
    ...
    void reset_state() { m_.clear(); v_.clear(); step_ = 0; }

private:
    float lr_, b1_, b2_, eps_, wd_; ClipPolicy clip_;
    uint64_t step_ = 0;
...
private:
    float lr_, eps_, wd_; ClipPolicy clip_;
...
public:
    ...
    void step(const ParamList& params) {
        ...
        for (auto* p : params) {
            ...
            const auto& g = p->grad();
            const size_t n = w.size();
            auto& sq = sq_avg_[p]; auto& buf = buf_[p];
            if (sq.size() != n) { sq.assign(n, 1.0f); buf.assign(n, 0.0f); }
            for (size_t i = 0; i < n; ++i) {
                float gi = g.data()[i] + wd_ * w.data()[i];
...
private:
    float lr_, alpha_, eps_, wd_, momentum_; ClipPolicy clip_;
