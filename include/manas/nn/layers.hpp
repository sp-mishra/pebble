...
public:
    ...
    TensorVar forward(const TensorVar& x, bool training = true) {
        ...
        if (training) {
            ...
            mean = running_mean_; var = running_var_;
        }
                ...
                [&tape, x_id, g_id, b_id, n, f,
                 ...
                 gdata = gamma_->data()](const Tensor& g) {
                    ...
                    if (g_id != Tape::kNoGrad) {
                        ...
                        for (size_t i = 0; i < n; ++i)
                            for (size_t j = 0; j < f; ++j) dg.data()[j] += g({i,j}) * xh({i,j});
                    ...
                    if (x_id != Tape::kNoGrad) {
                        ...
                        for (size_t j = 0; j < f; ++j) {
                            ...
                            for (size_t i = 0; i < n; ++i) { sum_g += g({i,j}); sum_gxh += g({i,j}) * xh({i,j}); }
                            for (size_t i = 0; i < n; ++i)
                                dx({i,j}) = gdata.data()[j] * invs.data()[j] * inv_n *
                                            (static_cast<float>(n) * g({i,j}) - sum_g - xh({i,j}) * sum_gxh);
                        }
    ...
    ParamList parameters() {
        ParamList p;
        p.push_back(gamma_.get()); p.push_back(beta_.get());
        return p;
    }
...
private:
    size_t num_features_; float eps_, momentum_; std::string name_;
...
public:
    ...
    TensorVar forward(const TensorVar& x, bool /*training*/ = true) {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            float var = 0.0f;
            for (size_t j = 0; j < f; ++j) { float d = x.data({i,j})-m; var += d*d; }
                ...
                [&tape, x_id, g_id, b_id, n, f,
                 ...
                 gdata = gamma_->data()](const Tensor& g_upstream) {
                    ...
                    if (g_id != Tape::kNoGrad) {
                        ...
                        for (size_t j = 0; j < f; ++j) dg.data()[j] = 0.0f;
                        for (size_t i = 0; i < n; ++i)
                            for (size_t j = 0; j < f; ++j) dg.data()[j] += g_upstream({i,j}) * xh({i,j});
                        tape.accumulate_grad(g_id, dg);
                    }
                    if (x_id != Tape::kNoGrad) {
                        Tensor dx({n, f});
...
private:
    size_t f_; float eps_; std::string name_;
    std::unique_ptr<Parameter> gamma_, beta_;
};
...
private:
    size_t vocab_, dim_; std::string name_;
