...
inline TensorVar mul(const TensorVar& a, const TensorVar& b) {
    ...
    if (tape.recording()) {
        ...
        out.tape_id = tape.push(true, out.data,
            [&tape, a_id, b_id,
             a_copy = std::move(a_data_copy), b_copy = std::move(b_data_copy)](const Tensor& g) {
                if (a_id != Tape::kNoGrad) {
                    Tensor ga(g.shape());
                    for (size_t i = 0; i < g.size(); ++i) ga.data()[i] = g.data()[i] * b_copy.data()[i];
                    tape.accumulate_grad(a_id, ga);
                }
                if (b_id != Tape::kNoGrad) {
                    Tensor gb(g.shape());
                    for (size_t i = 0; i < g.size(); ++i) gb.data()[i] = g.data()[i] * a_copy.data()[i];
...
inline TensorVar sum_all(const TensorVar& a) {
    ...
    TensorVar out(std::move(out_data));
    if (tape.recording()) {
        uint32_t a_id = a.tape_id; size_t n = a.data.size();
    ...
    if (tape.recording()) {
        ...
        out.tape_id = tape.push(true, out.data,
            [&tape, a_id, m = std::move(mask)](const Tensor& g) {
                if (a_id != Tape::kNoGrad) {
                    Tensor ga(g.shape());
                    for (size_t i = 0; i < g.size(); ++i) ga.data()[i] = g.data()[i] * m.data()[i];
                    tape.accumulate_grad(a_id, ga);
                }
            });
    }
...
inline TensorVar log_softmax(const TensorVar& a) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        for (size_t j = 0; j < c; ++j) max_v = std::max(max_v, a.data({i, j}));
        float sum_exp = 0.0f;
        for (size_t j = 0; j < c; ++j) { sm_data({i,j}) = std::exp(a.data({i,j}) - max_v); sum_exp += sm_data({i,j}); }
        for (size_t j = 0; j < c; ++j) { sm_data({i,j}) /= sum_exp; out_data({i,j}) = std::log(sm_data({i,j})); }
    }
