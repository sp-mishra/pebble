...
namespace prakriti {
    ...
    public:
        ...
        Index add(const ParticleDesc& d) {
            const Index i = size();
            pos_x.push_back(d.position[0]);
            pos_y.push_back(d.position[1]);
            pred_x.push_back(d.position[0]);
            pred_y.push_back(d.position[1]);
            vel_x.push_back(d.velocity[0]);
            vel_y.push_back(d.velocity[1]);
        ...
        [[nodiscard]] pebble::math::vec2 pos_v(Index i) const noexcept {
            return pebble::math::vec2(pos_x[i], pos_y[i]);
        }

        [[nodiscard]] pebble::math::vec2 pred_v(Index i) const noexcept {
            return pebble::math::vec2(pred_x[i], pred_y[i]);
        }

        [[nodiscard]] pebble::math::vec2 vel_v(Index i) const noexcept {
            return pebble::math::vec2(vel_x[i], vel_y[i]);
        }

        void set_pred(Index i, const pebble::math::vec2& v) noexcept {
            pred_x[i] = v[0];
            pred_y[i] = v[1];
        }
    };
