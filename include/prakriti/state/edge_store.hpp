...
public:
    ...
    void reserve(std::size_t n) {
        a.reserve(n); b.reserve(n); rest_len.reserve(n);
        strain.reserve(n); damage.reserve(n); active.reserve(n);
    }
    ...
    void compact() {
        ...
        for (Index r = 0; r < n; ++r) {
            ...
            if (w != r) {
                a[w] = a[r]; b[w] = b[r]; rest_len[w] = rest_len[r];
                strain[w] = strain[r]; damage[w] = damage[r]; active[w] = 1;
            }
            ++w;
        }
        a.resize(w); b.resize(w); rest_len.resize(w);
        strain.resize(w); damage.resize(w); active.resize(w);
    }
