...
namespace prakriti::celestial {
    ...
    compute_tde_spaghettification(float star_mass, float star_radius, float bh_mass, float distance) noexcept {
        ...
        if (distance < stream.tidal_radius && distance > 1.0f) {
            ...
            stream.accretion_feed_rate = std::clamp((stream.tidal_radius - distance) / stream.tidal_radius, 0.1f, 1.0f)
                * (star_mass * 0.45f);
        }
        return stream;
    }
    ...
    compute_osculating_orbit_points(const pebble::math::vec2& body_pos, const pebble::math::vec2& body_vel,
        ...
        for (int step = 0; step < num_steps; ++step) {
            ...
            const pebble::math::vec2 a = pebble::math::vec2{d[0] / dist, d[1] / dist} * ((18000.0f * host_mass) /
                dist2);
    ...
    evaluate_tidal_locking_torque(float m1, float r1, float omega1,
        ...
        res.spin_torque = -std::clamp(tidal_coeff * delta_omega * 0.015f * dt, -std::abs(delta_omega) * 0.5f,
                                      std::abs(delta_omega) * 0.5f);
    ...
    evaluate_jeans_atmospheric_escape(float planet_mass, float planet_radius, float planet_temp_celsius,
        ...
        const float lambda_jeans = (res.escape_velocity * res.escape_velocity) / (res.thermal_velocity * res.
        ...
        if (lambda_jeans >= 6.0f) {
            ...
            res.jeans_loss_rate = std::clamp((6.0f - lambda_jeans) * 0.02f * planet_mass * dt, 0.001f,
                                             planet_mass * 0.2f);
        }
    ...
    evaluate_pulsar_gw_timing_residual(float base_omega, float gw_strain, float gw_chirp_freq,
                                       float time_sec) noexcept {
    ...
    evaluate_sph_roche_lobe_stripping(const pebble::math::vec2& donor_pos, float donor_mass, float donor_radius,
        ...
        if (donor_radius > state.roche_lobe_radius) {
            ...
            const pebble::math::vec2 unit_d = (dir_len > 1e-3f)
                                                  ? (d * (1.0f / dir_len))
                                                  : pebble::math::vec2{1.0f, 0.0f};
    ...
    evaluate_snr_multiphase_expansion(float explosion_energy, float age_sec, float ism_density = 1.0f) noexcept {
        ...
        if (t < t_transition) {
            ...
            const float r_trans = 1.15f * std::pow(explosion_energy / std::max(ism_density, 0.1f), 0.2f) * std::pow(
                t_transition, 0.40f) * 65.0f;
            state.radius = r_trans * std::pow(t / t_transition, 0.25f);
            state.shock_velocity = (0.25f * state.radius) / t;
            state.post_shock_compression = 12.0f;
            // High radiative isothermal compression -> initiates protostellar collapse
