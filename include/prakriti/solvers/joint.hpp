...
namespace prakriti {
    ...
    private:
        ...
        void solve_distance(Store& P, Index ia, Index ib, Scalar wa, Scalar wb, Scalar rest, Scalar alpha) {
            ...
            P.pred_x[ia] += corr[0] * wa;
            P.pred_y[ia] += corr[1] * wa;
            P.pred_x[ib] -= corr[0] * wb;
            P.pred_y[ib] -= corr[1] * wb;
        }
        ...
        void solve_prismatic(Store& P, Index ia, Index ib, Scalar wa, Scalar wb,
            ...
            if (std::fabs(excess) > Scalar(1e-9)) {
                ...
                P.pred_x[ib] += corr[0] * (wb / denom);
                P.pred_y[ib] += corr[1] * (wb / denom);
                P.pred_x[ia] -= corr[0] * (wa / denom);
                P.pred_y[ia] -= corr[1] * (wa / denom);
            }
        }
    };
} // namespace prakriti
