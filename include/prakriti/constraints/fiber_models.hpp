...
struct HillTypeFiber {
    ...
    static void compute_force_batch(std::span<const Scalar> activation,
        ...
        const std::size_t n = std::min({activation.size(), lce.size(), vce.size(), lopt.size(), f_max.size(), pennation.size(), out_force.size()});
...
struct LinearFiber {
    ...
    static void compute_force_batch(std::span<const Scalar> activation,
        ...
        (void)vce;
        (void)vmax;
        const std::size_t n = std::min({activation.size(), lce.size(), lopt.size(), f_max.size(), pennation.size(), out_force.size()});
        for (std::size_t i = 0; i < n; ++i) {
            out_force[i] = compute_force(activation[i], lce[i], Scalar(0), lopt[i], Scalar(0), f_max[i], pennation[i]);
        }
    }
};
