...
namespace lang::samasa {
    ...
    struct railroad_rule_entry {
        ...
        // coarse shape tag: "sequence" | "choice" | "repetition" | "terminal" | "optional" | "other"
        bool nullable = false;
    };
    ...
    template <class... Ms>
    struct rr_shape_of<seq_t<Ms...>> {
        static constexpr std::string_view value = "sequence";
    };

    template <class... Ms>
    struct rr_shape_of<choice_t<Ms...>> {
        static constexpr std::string_view value = "choice";
    };

    template <class M>
    struct rr_shape_of<many_t<M>> {
        static constexpr std::string_view value = "repetition";
    };

    template <class M>
    struct rr_shape_of<many1_t<M>> {
        static constexpr std::string_view value = "repetition";
    };

    template <class M>
    struct rr_shape_of<opt_t<M>> {
        static constexpr std::string_view value = "optional";
    };

    template <auto Kind>
    struct rr_shape_of<tok<Kind>> {
        static constexpr std::string_view value = "terminal";
    };

    template <class A, class Sep>
    struct rr_shape_of<sep_by_t<A, Sep>> {
        static constexpr std::string_view value = "separated";
    };

    template <class A, class Sep>
    struct rr_shape_of<sep_by1_t<A, Sep>> {
        static constexpr std::string_view value = "separated";
    };
