...
namespace lang::samasa { namespace detail {
        ...
        template <class... Ms, class TokenKind>
        struct first_count<choice_t < Ms...>
        ,
        TokenKind
        >
        :
        ...
        struct first_count<rule < Name, Pattern>
        ,
        TokenKind
        >
        :
        ...
        struct first_count<node_t < Kind, Pattern>
        ,
        TK
        >
        :
        ...
        consteval std::size_t fill_follow_for_name(
            ...
            const std::array<TK, N>& cf, std::size_t cc,
            std::array<TK, N>& arr, std::size_t idx) { return fill_follow_for_name(target, M{}, cf, cc, arr, idx); }
    ...
    [[nodiscard]] consteval auto follow_of() {
        ...
        for (std::size_t i = 0; i < all_entries.size(); ++i)
            if (all_entries[i].name == Rule::name_sv) {
                found_idx = i;
                break;
            }
