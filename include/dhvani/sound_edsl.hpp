...
public:
    ...
    SoundBuilder& material(const physical::MaterialParams& m) noexcept {
        event_.material = m; return *this;
    }

    template <physical::PhysicalMaterial M>
    SoundBuilder& material(const M& m) noexcept {
        event_.material = m.material_params(); return *this;
    }

    SoundBuilder& force(float f)       noexcept { event_.force       = f;  return *this; }
    SoundBuilder& velocity(float v)    noexcept { event_.velocity    = v;  return *this; }
    SoundBuilder& metal(physical::MetalType t) noexcept { event_.metal_type = t;  return *this; }
    SoundBuilder& fundamental(float hz) noexcept { event_.fundamental = hz; return *this; }
    SoundBuilder& sample_rate(uint32_t sr) noexcept { event_.sample_rate = sr; return *this; }
    ...
    void render_into(std::span<synth::SampleFrame> out) const noexcept {
        ...
        switch (event_.type) {
            case SoundEventType::Impact: {
                ...
                for (auto& f : out) { const float s = v.tick(); f = {s, s}; }
                break;
            }
