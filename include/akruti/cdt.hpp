...
namespace akruti {
    struct CdtTriangulator {
        ...
        [[nodiscard]] auto operator()(const std::span<const Vec> poly) const {
            return triangulate(poly);
        }
        ...
        triangulate(const std::span<const Vec> vertices) const {
        ...
        template <std::size_t N>
        [[nodiscard]] auto triangulate(const containers::static_vector<Vec, N>& vertices) const {
            constexpr std::size_t MaxTris = (N >= 3) ? (N - 2) : 0;
            containers::static_vector<Triangle, MaxTris> tris;
            triangulate_into(std::span<const Vec>(vertices.data(), vertices.size()), tris);
            return tris;
        }

        // Overload 3: in-place buffer sink (zero allocations across frames in hot loops)
        template <typename OutContainer>
        void triangulate_into(const std::span<const Vec> vertices, OutContainer& out) const {
            const std::size_t n = vertices.size();
            if (n < 3) return;
            if (n == 3) {
                (void)out.push_back(Triangle{.a = vertices[0], .b = vertices[1], .c = vertices[2]});
                return;
            }
            ...
            for (std::size_t i = 1; i + 1 < n; ++i) {
                (void)out.push_back(Triangle{.a = v0, .b = vertices[i], .c = vertices[i + 1]});
            }
        }
    };
} // namespace akruti
