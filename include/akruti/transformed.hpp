...
struct TransformedShape {
    ...
    [[nodiscard]] constexpr AABB<Scalar> aabb() const noexcept {
        ...
        if (std::fabs(angle) < Scalar(1e-7)) {
            ...
            return AABB<Scalar>{pebble::math::vec2(local.lo[0] + pos_vec.x, local.lo[1] + pos_vec.y),
        ...
        return AABB<Scalar>{pebble::math::vec2(world_center.x - rotated_half.x, world_center.y - rotated_half.y),
