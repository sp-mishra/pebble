...
namespace akruti {
    ...
    [[nodiscard]] inline Vec sdf_gradient(const Circle& c, const Vec p) noexcept {
        const Vec diff = p - c.center;
        const Scalar d = akruti::length(diff);
        return d > static_cast<Scalar>(1e-6) ? (diff / d) : Vec{0, 1};
    }
    ...
    [[nodiscard]] inline Vec sdf_gradient(const Box& b, Vec p) noexcept {
        ...
        if (x(d) > 0.0f || y(d) > 0.0f) {
            const auto clamped = Vec{std::max(x(d), 0.0f), std::max(y(d), 0.0f)};
    ...
    [[nodiscard]] inline Vec sdf_gradient(const Capsule& cap, const Vec p) noexcept {
        const Vec ab = cap.b - cap.a;
        const Vec ap = p - cap.a;
        const Scalar t = std::clamp(akruti::dot(ap, ab) / std::max(akruti::length_sq(ab), static_cast<Scalar>(1e-12)), static_cast<Scalar>(0), static_cast<Scalar>(1));
    ...
    [[nodiscard]] inline Vec sdf_gradient(const OrientedBox& obb, const Vec p) noexcept {
        ...
        const Box local_box{.center = {0, 0}, .half = obb.half};
        const Vec local_grad = sdf_gradient(local_box, local);
        // Rotate back to world: R * local_grad
        return obb.rot * local_grad;
    }

    [[nodiscard]] inline Vec sdf_gradient(const RoundedBox& rb, const Vec p) noexcept {
        const Box inner{
            .center = rb.center, .half = Vec{
    ...
    [[nodiscard]] inline Vec sdf_gradient(const Segment& s, const Vec p) noexcept {
        const Vec ab = s.b - s.a;
        const Vec ap = p - s.a;
        const Scalar t = std::clamp(akruti::dot(ap, ab) / std::max(akruti::length_sq(ab), static_cast<Scalar>(1e-12)), static_cast<Scalar>(0), static_cast<Scalar>(1));
        const Vec closest = s.a + ab * t;
        const Vec diff = p - closest;
        if (const Scalar d = akruti::length(diff); d > static_cast<Scalar>(1e-6)) return diff / d;
    ...
    template <Shape S>
    [[nodiscard]] inline Vec sdf_gradient_fd(const S& shape, Vec p, const Scalar h = 1e-3f) noexcept {
    ...
    [[nodiscard]] inline Vec sdf_gradient(const TransformedShape<S>& ts, Vec p) noexcept {
        if (std::fabs(ts.angle) < static_cast<Scalar>(1e-7)) {
