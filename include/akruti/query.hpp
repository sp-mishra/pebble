...
namespace akruti {
    ...
    template <Shape S>
    [[nodiscard]] bool point_inside(const S& s, Vec p) noexcept {
        return s.sdf(p) < static_cast<Scalar>(0);
    }
    ...
    template <Shape S>
    [[nodiscard]] Vec closest_point(const S& s, Vec p,
                                    const Scalar h = 1e-3) noexcept {
    ...
    template <Shape S>
    [[nodiscard]] Vec project(const S& s, Vec p,
                              Scalar h = 1e-3) noexcept {
        if (s.sdf(p) <= static_cast<Scalar>(0)) return p; // already feasible
    ...
    template <Shape S>
    [[nodiscard]] Vec sdf_normal(const S& s, Vec p, const Scalar h = 1e-4) noexcept {
        const Scalar dx = s.sdf(Vec{p.x() + h, p.y()}) - s.sdf(Vec{p.x() - h, p.y()});
        const Scalar dy = s.sdf(Vec{p.x(), p.y() + h}) - s.sdf(Vec{p.x(), p.y() - h});
        const Vec g{dx, dy};
        const Scalar len = length(g);
        return (len > static_cast<Scalar>(1e-12)) ? g * (static_cast<Scalar>(1) / len) : Vec{0, 1};
    }

    // ── Principal Curvature (divergence of normal field: kappa = ∇·(∇f / |∇f|)) ───────
    template <Shape S>
    [[nodiscard]] Scalar sdf_curvature(const S& s, Vec p, const Scalar h = 1e-2) noexcept {
        const auto hd = static_cast<double>(h);
        const auto f0 = static_cast<double>(s.sdf(p));
        ...
        if (grad_sq < 1e-12) return static_cast<Scalar>(0);
    ...
    template <Shape S>
    [[nodiscard]] RayHit raycast(const S& s, const Vec origin, const Vec dir,
                                 const Scalar tmax = 1e4, const Scalar eps = 1e-4) noexcept {
        const Vec nd = akruti::normalize(dir);
        if (length_sq(nd) < static_cast<Scalar>(1e-18)) return RayHit{};
        Scalar t = 0;
    ...
    template <class It>
    [[nodiscard]] int winding_number(Vec p, It first, It last) noexcept {
        int wn = 0;
        ...
        for (It i = first; i != last; ++i) {
            ...
            if (a.y() <= p.y()) {
                if (b.y() > p.y() && cross(b - a, p - a) > static_cast<Scalar>(0)) ++wn;
            }
            else {
                if (b.y() <= p.y() && cross(b - a, p - a) < static_cast<Scalar>(0)) --wn;
            }
    ...
    [[nodiscard]] int winding_number(Vec p, const ConvexPoly<N>& poly) noexcept {
