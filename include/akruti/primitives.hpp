...
namespace akruti {
    ...
    struct Circle {
        ...
        [[nodiscard]] Scalar sdf(const Vec p) const noexcept { return distance(p, center) - radius; }

        [[nodiscard]] constexpr Box2 aabb() const noexcept {
            return {{center[0] - radius, center[1] - radius}, {center[0] + radius, center[1] + radius}};
        }

        [[nodiscard]] Vec support(const Vec d) const noexcept { return center + akruti::normalize(d) * radius; }
    ...
    struct Segment {
        ...
        [[nodiscard]] Scalar sdf(const Vec p) const noexcept {
            ...
            const Scalar t = std::clamp(akruti::dot(ap, ab) / std::max(ab_len_sq, static_cast<Scalar>(1e-12)),
        ...
        [[nodiscard]] constexpr Vec support(const Vec d) const noexcept {
            return akruti::dot(d, a) >= akruti::dot(d, b) ? a : b;
        }

        [[nodiscard]] constexpr Vec centroid() const noexcept { return (a + b) * static_cast<Scalar>(0.5); }
    };
    ...
    struct Capsule {
        ...
        [[nodiscard]] Scalar sdf(const Vec p) const noexcept { return Segment{.a = a, .b = b}.sdf(p) - radius; }
    ...
    struct Box {
        ...
        [[nodiscard]] Scalar sdf(Vec p) const noexcept {
            ...
            const Vec qp{std::max(q[0], static_cast<Scalar>(0)), std::max(q[1], static_cast<Scalar>(0))};
            return length(qp) + std::min(std::max(q[0], q[1]), static_cast<Scalar>(0));
        }
    ...
    struct OrientedBox {
        ...
        [[nodiscard]] static OrientedBox from_angle(const Vec center, const Vec half, const Scalar radians) noexcept {
            return {.center = center, .half = half, .rot = akruti::make_rotation2d<Scalar>(radians)};
        }
    ...
    struct Triangle {
        ...
        [[nodiscard]] Scalar sdf(const Vec p) const noexcept {
            ...
            const Vec pq0 = v0 - e0 * std::clamp(
                akruti::dot(v0, e0) / std::max(length_sq(e0), static_cast<Scalar>(1e-12)), static_cast<Scalar>(0),
                static_cast<Scalar>(1));
            const Vec pq1 = v1 - e1 * std::clamp(
                akruti::dot(v1, e1) / std::max(length_sq(e1), static_cast<Scalar>(1e-12)), static_cast<Scalar>(0),
                static_cast<Scalar>(1));
            const Vec pq2 = v2 - e2 * std::clamp(
                akruti::dot(v2, e2) / std::max(length_sq(e2), static_cast<Scalar>(1e-12)), static_cast<Scalar>(0),
                static_cast<Scalar>(1));

            const Scalar s = cross(e0, -e2); // Area / orientation: positive if a->b->c is CCW
            const Scalar d = std::min({length_sq(pq0), length_sq(pq1), length_sq(pq2)});
        ...
        [[nodiscard]] constexpr Vec centroid() const noexcept {
            return (a + b + c) * (static_cast<Scalar>(1) / static_cast<Scalar>(3));
        }
    };
    ...
    struct RoundedBox {
        ...
        [[nodiscard]] Scalar sdf(Vec p) const noexcept {
            ...
            const Vec qp{std::max(q[0], static_cast<Scalar>(0)), std::max(q[1], static_cast<Scalar>(0))};
            return length(qp) + std::min(std::max(q[0], q[1]), static_cast<Scalar>(0)) - radius;
        }
        ...
        [[nodiscard]] Vec support(Vec d) const noexcept {
            const Vec inner_half{
                std::max(static_cast<Scalar>(0), half[0] - radius), std::max(static_cast<Scalar>(0), half[1] - radius)
            };
            const Vec b_sup{
    ...
    struct Sector {
        ...
        [[nodiscard]] static Sector from_direction(const Vec center, const Scalar r, const Scalar half_ang,
                                                   const Vec dir) noexcept {
            const Vec nd = akruti::normalize(dir);
            // Rotation aligning +X with nd
            return {
                .center = center, .radius = r, .half_angle = half_ang, .rot = Mat2<Scalar>{nd[0], -nd[1], nd[1], nd[0]}
            };
        }
        ...
        [[nodiscard]] Scalar sdf(const Vec p) const noexcept {
            ...
            if (const Scalar z = p_rot[0] * cs[0] - p_rot[1] * cs[1]; z <= 0) {
                return len - radius;
            }
            const Vec edge = cs * std::clamp(akruti::dot(q, Vec{cs[0], cs[1]}), static_cast<Scalar>(0), radius);
            return length(q - Vec{edge[0], std::copysign(edge[1], q[1])});
        }
        ...
        [[nodiscard]] Vec support(Vec d) const noexcept {
            ...
            if (local_d.x() > 0.0f) {
                if (const Scalar ang = std::atan2(local_d.y(), local_d.x()); std::fabs(ang) <= half_angle) {
                    const Vec p_arc = akruti::normalize(local_d) * radius;
                    if (const Scalar dot_arc = akruti::dot(p_arc, local_d); dot_arc > best_dot) {
                        best_dot = dot_arc;
                        best_p = p_arc;
                    }
                }
    ...
    struct HalfPlane {
        ...
        [[nodiscard]] Scalar sdf(const Vec p) const noexcept {
            return akruti::dot(akruti::normalize(normal), p - point);
        }

        [[nodiscard]] constexpr Box2 aabb() const noexcept {
            constexpr auto big = static_cast<Scalar>(1e18);
            return {{-big, -big}, {big, big}};
        }

        [[nodiscard]] Vec support(const Vec d) const noexcept {
            // Unbounded; project a far point against the ray direction.
            return point - akruti::normalize(normal) * (akruti::dot(d, normal) > 0
                                                            ? static_cast<Scalar>(0)
                                                            : static_cast<Scalar>(1e18));
        }
    ...
    struct ConvexPoly {
        ...
        [[nodiscard]] Scalar sdf(Vec p) const noexcept {
            ...
            for (std::size_t i = 0, j = n - 1; i < n; j = i++) {
                ...
                const Scalar t = std::clamp(akruti::dot(w, e) / std::max(length_sq(e), static_cast<Scalar>(1e-12)),
                                            static_cast<Scalar>(0), static_cast<Scalar>(1));
                const Vec proj = w - e * t;
                d = std::min(d, length_sq(proj));
                // winding sign test (point-in-convex via edge crossings)
                const bool c1 = p.y() >= verts[i].y(), c2 = p.y() < verts[j].y();
                if (const bool c3 = cross(e, w) > 0; (c1 && c2 && c3) || (!c1 && !c2 && !c3)) s = -s;
            }
            return s * std::sqrt(d);
        }
        ...
        [[nodiscard]] Vec support(Vec d) const noexcept {
            ...
            for (std::size_t i = 1; i < verts.size(); ++i) {
                if (const Scalar dp = akruti::dot(d, verts[i]); dp > bestDot) {
                    bestDot = dp;
                    best = i;
                }
            }
        ...
        [[nodiscard]] Vec centroid() const noexcept {
            ...
            area *= static_cast<Scalar>(0.5);
            if (std::fabs(area) < static_cast<Scalar>(1e-12)) {
                Vec sum{};
                for (const auto& v : verts) sum = sum + v;
                return sum * (static_cast<Scalar>(1) / static_cast<Scalar>(n));
            }
            const Scalar inv = static_cast<Scalar>(1) / (static_cast<Scalar>(6) * area);
    ...
    struct ChainShape {
        ...
        [[nodiscard]] Scalar sdf(const Vec p) const noexcept {
            ...
            for (std::size_t i = 0; i < seg_count; ++i) {
                ...
                const Scalar t = std::clamp(akruti::dot(w, e) / std::max(length_sq(e), static_cast<Scalar>(1e-12)),
                                            static_cast<Scalar>(0), static_cast<Scalar>(1));
                const Vec proj = w - e * t;
                min_d2 = std::min(min_d2, length_sq(proj));
            }
        ...
        [[nodiscard]] Vec edge_normal(std::size_t i) const noexcept {
            ...
            const Vec v1 = verts[(i + 1) % n];
            const Vec e = v1 - v0;
            return akruti::normalize(perp(e)); // Outward CCW 90-degree normal
        }
        ...
        [[nodiscard]] Vec centroid() const noexcept {
            ...
            Vec sum{};
            for (const auto& v : verts) sum = sum + v;
            return sum * (static_cast<Scalar>(1) / static_cast<Scalar>(verts.size()));
        }
    };
    ...
    struct GridSDF {
        ...
        [[nodiscard]] Scalar sample(const std::size_t x, const std::size_t y) const noexcept {
        ...
        [[nodiscard]] Scalar sdf(Vec p) const noexcept {
            ...
            const Box box{.center = center, .half = extent * 0.5f};
        ...
        [[nodiscard]] Vec support(const Vec d) const noexcept {
            const Box box{.center = bounds.center(), .half = bounds.extent() * 0.5f};
