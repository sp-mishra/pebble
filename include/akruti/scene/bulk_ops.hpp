...
struct PairId    { std::uint32_t a{}, b{}; };
struct NearestHit{ std::uint32_t payload{}; Scalar dist{}; Vec2<Scalar> point{}; };
struct Ray       { Vec2<Scalar> o{}, d{}; Scalar tmax{Scalar(1e4)}; };
struct GridSpec  { Vec2<Scalar> origin{}; Vec2<Scalar> cell{Scalar(1), Scalar(1)}; std::uint32_t nx{}, ny{}; };
    ...
    for (int pass = 0; pass < 40; ++pass) {
        ...
        scene.tree().query(q, [&](std::uint32_t payload) {
            scene.dispatch(payload, [&](const auto& batch, std::uint32_t idx) {
                ...
                if (d < best.dist) { best.dist = d; best.payload = payload; best.point = p; }
            });
        });
...
inline void bulk_sdf_field(Scene& scene, const GridSpec& grid, std::span<Scalar> out) {
    ...
    scene.executor().for_range(total, [&](std::size_t i) {
        ...
        const Vec2<Scalar> p{grid.origin.x + Scalar(ix) * grid.cell.x,
