...
struct Handle { std::uint32_t payload{}; };
...
public:
    ...
    template <class Prim> [[nodiscard]] ShapeBatch<Prim>&       batch()       noexcept { return std::get<ShapeBatch<Prim>>(batches_); }
    template <class Prim> [[nodiscard]] const ShapeBatch<Prim>& batch() const noexcept { return std::get<ShapeBatch<Prim>>(batches_); }
    ...
    template <class Prim> [[nodiscard]] std::size_t count() const noexcept { return batch<Prim>().size(); }

    template <class Fn> void for_each_leaf(Fn&& fn) const {
    ...
    template <class Visitor> void dispatch(std::uint32_t payload, Visitor&& v) const {
...
private:
    ...
    template <class Fn> void for_each_batch(Fn&& fn) const {
    ...
    template <class Fn> void for_each_batch_mut(Fn&& fn) {
