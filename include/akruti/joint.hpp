...
[[nodiscard]] inline Joint make_distance(std::uint32_t a, std::uint32_t b, Vec2<Scalar> la,
    ...
    Joint j; j.type = JointType::Distance; j.body_a = a; j.body_b = b;
    j.anchor_a = la; j.anchor_b = lb; j.rest_length = rest; j.compliance = compliance;
    return j;
}
[[nodiscard]] inline Joint make_revolute(std::uint32_t a, std::uint32_t b, Vec2<Scalar> la,
    ...
    Joint j; j.type = JointType::Revolute; j.body_a = a; j.body_b = b;
    j.anchor_a = la; j.anchor_b = lb; j.compliance = compliance;
    return j;
}
[[nodiscard]] inline Joint make_prismatic(std::uint32_t a, std::uint32_t b, Vec2<Scalar> la,
    ...
    Joint j; j.type = JointType::Prismatic; j.body_a = a; j.body_b = b;
    j.anchor_a = la; j.anchor_b = lb; j.axis = axis.normalized();
    j.min_limit = lo; j.max_limit = hi; j.compliance = compliance;
    return j;
}
[[nodiscard]] inline Joint make_weld(std::uint32_t a, std::uint32_t b, Vec2<Scalar> la,
                                     Vec2<Scalar> lb, Scalar compliance = 0) noexcept {
    Joint j; j.type = JointType::Weld; j.body_a = a; j.body_b = b;
    j.anchor_a = la; j.anchor_b = lb; j.compliance = compliance;
    return j;
}
[[nodiscard]] inline Joint make_motor(std::uint32_t a, std::uint32_t b, Scalar target,
                                      Scalar lo, Scalar hi, Scalar compliance = 0) noexcept {
    Joint j; j.type = JointType::Motor; j.body_a = a; j.body_b = b;
    j.target_angle = target; j.min_limit = lo; j.max_limit = hi; j.compliance = compliance;
    return j;
}
