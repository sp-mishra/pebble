...
namespace akruti {
    ...
    [[nodiscard]] inline Manifold collide_circle_circle(const Circle& a, const Circle& b) noexcept {
        ...
        const Scalar dist2 = akruti::length_sq(diff);
    ...
    [[nodiscard]] inline Manifold collide_circle_box(const Circle& a, const Box& b) noexcept {
        ...
        const Scalar d2 = akruti::length_sq(diff);
    ...
    [[nodiscard]] inline Manifold collide_circle_obb(const Circle& a, const OrientedBox& b) noexcept {
        ...
        Manifold m;
        m.hit = true;
        m.normal = akruti::mul(b.rot, m_local.normal);
        m.depth = m_local.depth;
        for (std::size_t i = 0; i < m_local.points.size(); ++i) {
            (void)m.points.push_back(ContactPoint{b.center + akruti::mul(b.rot, m_local.points[i].point), m_local.points[i].depth});
        }
        return m;
    }
    ...
    [[nodiscard]] inline Manifold collide_circle_triangle(const Circle& a, const Triangle& b) noexcept {
        ...
        Scalar min_d2 = akruti::length_sq(pq0);
        if (akruti::length_sq(pq1) < min_d2) {
            min_d2 = akruti::length_sq(pq1);
            closest = b.b + (v1 - pq1);
        }
        if (akruti::length_sq(pq2) < min_d2) {
            min_d2 = akruti::length_sq(pq2);
            closest = b.c + (v2 - pq2);
        }

        Vec diff = closest - a.center;
        Scalar d = akruti::length(diff);
    ...
    [[nodiscard]] inline Manifold collide_circle_sector(const Circle& a, const Sector& b) noexcept {
        ...
        const Vec diff = b.center - a.center;
        Vec n = akruti::length_sq(diff) > 1e-6f ? akruti::normalize(diff) : Vec{1, 0};
    ...
    [[nodiscard]] inline Manifold collide_segment_box(const Segment& a, const Box& b) noexcept {
        ...
        const OrientedBox obb{b.center, b.half, Mat2{1, 0, 0, 1}};
    ...
    [[nodiscard]] inline Manifold collide_capsule_capsule(const Capsule& a, const Capsule& b) noexcept {
        ...
        const Scalar a_len2 = akruti::length_sq(d1);
        const Scalar b_len2 = akruti::length_sq(d2);
    ...
    [[nodiscard]] inline Manifold collide_capsule_obb(const Capsule& a, const OrientedBox& b) noexcept {
        ...
        if (!m_a.hit && !m_b.hit) {
            ...
            Manifold world_m;
            world_m.hit = true;
            world_m.normal = akruti::mul(b.rot, m_m.normal);
            world_m.depth = m_m.depth;
            (void)world_m.points.push_back(ContactPoint{b.center + akruti::mul(b.rot, m_m.points[0].point), m_m.depth});
            return world_m;
        }
        ...
        if (m_a.hit) {
            world_m.normal = akruti::mul(b.rot, m_a.normal);
            world_m.depth = m_a.depth;
            (void)world_m.points.push_back(ContactPoint{b.center + akruti::mul(b.rot, m_a.points[0].point), m_a.depth});
        }

        if (m_b.hit) {
            const Vec norm_b = akruti::mul(b.rot, m_b.normal);
            ...
            (void)world_m.points.push_back(ContactPoint{b.center + akruti::mul(b.rot, m_b.points[0].point), m_b.depth});
        }
    ...
    [[nodiscard]] inline Manifold collide_triangle_triangle(const Triangle& a, const Triangle& b) noexcept {
        ...
        if (akruti::dot(best_axis, center_b - center_a) < 0.0f) best_axis = -best_axis;
    ...
    [[nodiscard]] inline Manifold collide_obb_triangle(const OrientedBox& a, const Triangle& b) noexcept {
        ...
        const Vec center_b = (b.a + b.b + b.c) * (1.0f / 3.0f);
        if (akruti::dot(best_axis, center_b - a.center) < 0.0f) best_axis = -best_axis;
    ...
    [[nodiscard]] inline Manifold collide_obb_obb(const OrientedBox& a, const OrientedBox& b) noexcept {
        ...
        for (const Vec& axis : axes) {
            const Scalar ra = std::fabs(akruti::dot(ax, axis)) * a.half.x() + std::fabs(akruti::dot(ay, axis)) * a.half.y();
            const Scalar rb = std::fabs(akruti::dot(bx, axis)) * b.half.x() + std::fabs(akruti::dot(by, axis)) * b.half.y();
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            const Scalar dot = akruti::dot(n, best_axis);
            if (dot > max_dot) {
                max_dot = dot;
                ref_idx = i;
            }
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            const Vec e = v_next - v_curr;
            const Vec n = akruti::normalize(Vec{e.y(), -e.x()});
            const Scalar dot = akruti::dot(n, ref_normal);
            if (dot < min_inc_dot) {
                min_inc_dot = dot;
                inc_idx = i;
            }
        ...
        for (int i = 0; i < count2; ++i) {
            const Scalar sep = akruti::dot(ref_normal, clip2[static_cast<std::size_t>(i)]) - ref_depth;
            if (sep <= 0.05f) {
    ...
    [[nodiscard]] inline Manifold collide_box_box(const Box& a, const Box& b) noexcept {
        const OrientedBox obb_a{a.center, a.half, Mat2{1, 0, 0, 1}};
        const OrientedBox obb_b{b.center, b.half, Mat2{1, 0, 0, 1}};
