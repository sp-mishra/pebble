...
namespace akruti {
    ...
    [[nodiscard]] inline Manifold collide_circle_circle(const Circle& a, const Circle& b) noexcept {
        ...
        m.normal = n;
        m.depth = depth;
        (void)m.points.push_back(ContactPoint{.point = cp, .depth = depth});
        return m;
    }
    ...
    [[nodiscard]] inline Manifold collide_circle_box(const Circle& a, const Box& b) noexcept {
        ...
        if (const Scalar d2 = length_sq(diff); d2 > static_cast<Scalar>(1e-9)) {
            ...
            m.normal = n;
            m.depth = depth;
            (void)m.points.push_back(ContactPoint{.point = a.center - n * a.radius, .depth = depth});
            return m;
        }
    }
    ...
    [[nodiscard]] inline Manifold collide_circle_obb(const Circle& a, const OrientedBox& b) noexcept {
        ...
        const Vec local_c{
            b.rot(0, 0) * diff.x() + b.rot(1, 0) * diff.y(),
            b.rot(0, 1) * diff.x() + diff.y() * b.rot(1, 1)
        };
        const Circle local_circle{.center = local_c, .radius = a.radius};
        const Box local_box{.center = {0, 0}, .half = b.half};

        auto [m_local_hit, m_local_normal, m_local_points, m_local_depth] = collide_circle_box(local_circle, local_box);
        if (!m_local_hit) return Manifold{};

        Manifold m;
        m.hit = true;
        m.normal = akruti::mul(b.rot, m_local_normal);
        m.depth = m_local_depth;
        for (auto& [point, depth] : m_local_points) {
            (void)m.points.push_back(ContactPoint{.point = b.center + akruti::mul(b.rot, point), .depth = depth});
        }
        return m;
    }
    ...
    [[nodiscard]] inline Manifold collide_circle_triangle(const Circle& a, const Triangle& b) noexcept {
        ...
        const Vec pq0 = v0 - e0 * std::clamp(akruti::dot(v0, e0) / std::max(length_sq(e0), static_cast<Scalar>(1e-12)),
                                             static_cast<Scalar>(0), static_cast<Scalar>(1));
        const Vec pq1 = v1 - e1 * std::clamp(akruti::dot(v1, e1) / std::max(length_sq(e1), static_cast<Scalar>(1e-12)),
                                             static_cast<Scalar>(0), static_cast<Scalar>(1));
        const Vec pq2 = v2 - e2 * std::clamp(akruti::dot(v2, e2) / std::max(length_sq(e2), static_cast<Scalar>(1e-12)),
                                             static_cast<Scalar>(0), static_cast<Scalar>(1));

        Vec closest = b.a + (v0 - pq0);
        Scalar min_d2 = length_sq(pq0);
        if (length_sq(pq1) < min_d2) {
            min_d2 = length_sq(pq1);
            closest = b.b + (v1 - pq1);
        }
        if (length_sq(pq2) < min_d2) {
            min_d2 = length_sq(pq2);
            closest = b.c + (v2 - pq2);
        }

        const Vec diff = closest - a.center;
        const Scalar d = length(diff);
        ...
        m.normal = n;
        m.depth = depth;
        (void)m.points.push_back(ContactPoint{.point = closest, .depth = depth});
        return m;
    }
    ...
    [[nodiscard]] inline Manifold collide_circle_roundedbox(const Circle& a, const RoundedBox& b) noexcept {
        const Box inner{
            .center = b.center,
            .half = Vec{std::max(0.0f, b.half.x() - b.radius), std::max(0.0f, b.half.y() - b.radius)}
        };
        const Circle expanded_a{.center = a.center, .radius = a.radius + b.radius};
        const Manifold m = collide_circle_box(expanded_a, inner);
    ...
    [[nodiscard]] inline Manifold collide_circle_sector(const Circle& a, const Sector& b) noexcept {
        ...
        const Vec diff = b.center - a.center;
        const Vec n = length_sq(diff) > 1e-6f ? akruti::normalize(diff) : Vec{1, 0};

        Manifold m;
        m.hit = true;
        m.normal = n;
        m.depth = depth;
        (void)m.points.push_back(ContactPoint{.point = a.center + n * (a.radius - depth * 0.5f), .depth = depth});
        return m;
    }
    ...
    [[nodiscard]] inline Manifold collide_segment_circle(const Segment& a, const Circle& b) noexcept {
        ...
        const Scalar d2 = length_sq(diff);
    ...
    [[nodiscard]] inline Manifold collide_segment_box(const Segment& a, const Box& b) noexcept {
        ...
        const Capsule cap{.a = a.a, .b = a.b, .radius = 0.0f};
        const OrientedBox obb{.center = b.center, .half = b.half, .rot = Mat2{1, 0, 0, 1}};
    ...
    [[nodiscard]] inline Manifold collide_capsule_capsule(const Capsule& a, const Capsule& b) noexcept {
        ...
        const Scalar a_len2 = length_sq(d1);
        const Scalar b_len2 = length_sq(d2);
        const Scalar f = akruti::dot(d2, r);
        ...
        if (a_len2 <= 1e-6f && b_len2 <= 1e-6f) {
            ...
            const Scalar c = akruti::dot(d1, r);
            if (b_len2 <= 1e-6f) {
                t = 0.0f;
                s = std::clamp(-c / a_len2, 0.0f, 1.0f);
            }
            else {
                const Scalar b_dot = akruti::dot(d1, d2);
                if (const Scalar denom = a_len2 * b_len2 - b_dot * b_dot; denom > 1e-6f) {
        ...
        const Vec n = (dist > 1e-6f) ? (diff / dist) : Vec{0, 1};
        const Scalar depth = r_sum - dist;
        ...
        constexpr Scalar parallel_tol = 0.98f;
        const bool parallel = (std::fabs(akruti::dot(akruti::normalize(d1), akruti::normalize(d2))) > parallel_tol) &&
            (a_len2 > 1e-4f) && (b_len2 > 1e-4f);

        if (parallel) {
            const Vec d2_norm = akruti::normalize(d2);
            const Scalar t_a = std::clamp(akruti::dot(a.a - b.a, d2_norm) / std::sqrt(b_len2), 0.0f, 1.0f);
            const Vec proj_a = b.a + d2 * t_a;
            const Scalar d_a = length(proj_a - a.a);

            const Scalar t_b = std::clamp(akruti::dot(a.b - b.a, d2_norm) / std::sqrt(b_len2), 0.0f, 1.0f);
            const Vec proj_b = b.a + d2 * t_b;
            const Scalar d_b = length(proj_b - a.b);

            if (d_a < r_sum) {
                (void)m.points.push_back(ContactPoint{
                    .point = a.a + n * (a.radius - (r_sum - d_a) * 0.5f), .depth = r_sum - d_a
                });
            }
            if (d_b < r_sum && m.points.size() < 2) {
                (void)m.points.push_back(ContactPoint{
                    .point = a.b + n * (a.radius - (r_sum - d_b) * 0.5f), .depth = r_sum - d_b
                });
            }
        }
    ...
    [[nodiscard]] inline Manifold collide_capsule_obb(const Capsule& a, const OrientedBox& b) noexcept {
        ...
        const Vec local_a{
            b.rot(0, 0) * diff_a.x() + b.rot(1, 0) * diff_a.y(),
            b.rot(0, 1) * diff_a.x() + diff_a.y() * b.rot(1, 1)
        };

        const Vec diff_b = a.b - b.center;
        const Vec local_b{
            b.rot(0, 0) * diff_b.x() + b.rot(1, 0) * diff_b.y(),
            b.rot(0, 1) * diff_b.x() + diff_b.y() * b.rot(1, 1)
        };

        const Box local_box{.center = {0, 0}, .half = b.half};
        const Circle local_ca{.center = local_a, .radius = a.radius};
        const Circle local_cb{.center = local_b, .radius = a.radius};

        const auto [m_a_hit, m_a_normal, m_a_points, m_a_depth] = collide_circle_box(local_ca, local_box);
        const auto [m_b_hit, m_b_normal, m_b_points, m_b_depth] = collide_circle_box(local_cb, local_box);

        if (!m_a_hit && !m_b_hit) {
            const Vec local_mid = (local_a + local_b) * 0.5f;
            const Circle local_cm{.center = local_mid, .radius = a.radius};
            const auto [m_m_hit, m_m_normal, m_m_points, m_m_depth] = collide_circle_box(local_cm, local_box);
            if (!m_m_hit) return Manifold{};

            Manifold world_m;
            world_m.hit = true;
            world_m.normal = akruti::mul(b.rot, m_m_normal);
            world_m.depth = m_m_depth;
            (void)world_m.points.push_back(ContactPoint{
                .point = b.center + akruti::mul(b.rot, m_m_points[0].point), .depth = m_m_depth
            });
            return world_m;
        }
        ...
        if (m_a_hit) {
            world_m.normal = akruti::mul(b.rot, m_a_normal);
            world_m.depth = m_a_depth;
            (void)world_m.points.push_back(ContactPoint{
                .point = b.center + akruti::mul(b.rot, m_a_points[0].point), .depth = m_a_depth
            });
        }

        if (m_b_hit) {
            const Vec norm_b = akruti::mul(b.rot, m_b_normal);
            if (world_m.points.empty()) {
                world_m.normal = norm_b;
                world_m.depth = m_b_depth;
            }
            else {
                world_m.normal = akruti::normalize(world_m.normal + norm_b);
                world_m.depth = std::max(world_m.depth, m_b_depth);
            }
            (void)world_m.points.push_back(ContactPoint{
                .point = b.center + akruti::mul(b.rot, m_b_points[0].point), .depth = m_b_depth
            });
        }
    ...
    [[nodiscard]] inline Manifold collide_capsule_triangle(const Capsule& a, const Triangle& b) noexcept {
        const Capsule edge0{.a = b.a, .b = b.b, .radius = 0.0f};
        const Capsule edge1{.a = b.b, .b = b.c, .radius = 0.0f};
        const Capsule edge2{.a = b.c, .b = b.a, .radius = 0.0f};
    ...
    namespace detail {
        inline void clip_segment_to_line(std::array<Vec, 2>& out_pts, int& out_count,
                                         const std::array<Vec, 2>& in_pts, const Vec normal,
                                         const Scalar offset) noexcept {
            out_count = 0;
            const Scalar d0 = akruti::dot(normal, in_pts[0]) - offset;
            const Scalar d1 = akruti::dot(normal, in_pts[1]) - offset;
    ...
    [[nodiscard]] inline Manifold collide_triangle_triangle(const Triangle& a, const Triangle& b) noexcept {
        ...
        const std::array<Vec, 6> axes = {
            akruti::normalize(Vec{-(a.b.y() - a.a.y()), a.b.x() - a.a.x()}),
            akruti::normalize(Vec{-(a.c.y() - a.b.y()), a.c.x() - a.b.x()}),
            akruti::normalize(Vec{-(a.a.y() - a.c.y()), a.a.x() - a.c.x()}),
            akruti::normalize(Vec{-(b.b.y() - b.a.y()), b.b.x() - b.a.x()}),
            akruti::normalize(Vec{-(b.c.y() - b.b.y()), b.c.x() - b.b.x()}),
            akruti::normalize(Vec{-(b.a.y() - b.c.y()), b.a.x() - b.c.x()})
        };
        ...
        for (const Vec& axis : axes) {
            ...
            for (const Vec& v : a_verts) {
                const Scalar p = akruti::dot(v, axis);
        ...
        if (const Vec center_b = (b.a + b.b + b.c) * (1.0f / 3.0f); akruti::dot(best_axis, center_b - center_a) < 0.0f)
            best_axis = -best_axis;
        ...
        m.normal = best_axis;
        m.depth = min_overlap;
        (void)m.points.push_back(ContactPoint{.point = center_a + best_axis * min_overlap, .depth = min_overlap});
        return m;
    }
    ...
    [[nodiscard]] inline Manifold collide_obb_triangle(const OrientedBox& a, const Triangle& b) noexcept {
        const auto ax = Vec{a.rot(0, 0), a.rot(1, 0)};
        const auto ay = Vec{a.rot(0, 1), a.rot(1, 1)};
        ...
        if (const Vec center_b = (b.a + b.b + b.c) * (1.0f / 3.0f); akruti::dot(best_axis, center_b - a.center) < 0.0f)
            best_axis = -best_axis;
        ...
        m.normal = best_axis;
        m.depth = min_overlap;
        (void)m.points.push_back(ContactPoint{.point = a.center + best_axis * a.half.x(), .depth = min_overlap});
        return m;
    }
    ...
    [[nodiscard]] inline Manifold collide_obb_obb(const OrientedBox& a, const OrientedBox& b) noexcept {
        const auto ax = Vec{a.rot(0, 0), a.rot(1, 0)};
        const auto ay = Vec{a.rot(0, 1), a.rot(1, 1)};
        const auto bx = Vec{b.rot(0, 0), b.rot(1, 0)};
        const auto by = Vec{b.rot(0, 1), b.rot(1, 1)};
        ...
        for (const Vec& axis : axes) {
            const Scalar ra = std::fabs(akruti::dot(ax, axis)) * a.half.x() + std::fabs(akruti::dot(ay, axis)) * a.half.
                y();
            const Scalar rb = std::fabs(akruti::dot(bx, axis)) * b.half.x() + std::fabs(akruti::dot(by, axis)) * b.half.
                y();
            const Scalar dist = std::fabs(akruti::dot(diff, axis));
        ...
        if (akruti::dot(best_axis, diff) < 0.0f) best_axis = -best_axis;
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            const Vec n = akruti::normalize(Vec{e.y(), -e.x()});
            if (const Scalar dot = akruti::dot(n, best_axis); dot > max_dot) {
                max_dot = dot;
                ref_idx = i;
            }
        }
        ...
        const Vec ref_tangent = akruti::normalize(ref_v2 - ref_v1);
        const Vec ref_normal = akruti::normalize(Vec{ref_tangent.y(), -ref_tangent.x()});
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            const Vec e = v_next - v_curr;
            const Vec n = akruti::normalize(Vec{e.y(), -e.x()});
            if (const Scalar dot = akruti::dot(n, ref_normal); dot < min_inc_dot) {
                min_inc_dot = dot;
                inc_idx = i;
            }
        }
        std::array<Vec, 2> inc_pts = {
            b_verts[static_cast<std::size_t>(inc_idx)],
            b_verts[static_cast<std::size_t>((inc_idx + 1) % 4)]
        };

        std::array<Vec, 2> clip1{};
        int count1 = 0;
        detail::clip_segment_to_line(clip1, count1, inc_pts, -ref_tangent, akruti::dot(-ref_tangent, ref_v1));
        if (count1 < 2) count1 = 2;

        std::array<Vec, 2> clip2{};
        int count2 = 0;
        detail::clip_segment_to_line(clip2, count2, clip1, ref_tangent, akruti::dot(ref_tangent, ref_v2));
        ...
        const Scalar ref_depth = akruti::dot(ref_normal, ref_v1);
        for (int i = 0; i < count2; ++i) {
            if (const Scalar sep = akruti::dot(ref_normal, clip2[static_cast<std::size_t>(i)]) - ref_depth; sep <=
                0.05f) {
                (void)m.points.push_back(ContactPoint{
                    .point = clip2[static_cast<std::size_t>(i)], .depth = std::max(0.0f, -sep)
                });
            }
        }
    ...
    [[nodiscard]] inline Manifold collide_box_box(const Box& a, const Box& b) noexcept {
        const OrientedBox obb_a{.center = a.center, .half = a.half, .rot = Mat2{1, 0, 0, 1}};
        const OrientedBox obb_b{.center = b.center, .half = b.half, .rot = Mat2{1, 0, 0, 1}};
    ...
    [[nodiscard]] inline Manifold collide_gjk_warm_started(const A& a, const B& b,
        ...
        (cache && cache->valid) ? cache->separating_axis : Vec{1, 0};

        if (const bool overlap = gjk_overlap(a, b, &simp); !overlap) {
            if (cache) {
                cache->valid = false;
                cache->simplex.clear();
            }
        ...
        (void)m.points.push_back(ContactPoint{.point = cp, .depth = c.depth});
        return m;
    }
