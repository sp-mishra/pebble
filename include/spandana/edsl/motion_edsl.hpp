...
namespace pebble::spandana::edsl {
    ...
    public:
        Vec2SpringAction(pebble::math::vec2& prop, pebble::math::vec2 target, float stiffness, float damping,
                         float duration, ResourceKey key)
    ...
    struct Vec2SpringBuilder {
        ...
        [[nodiscard]] Vec2SpringAction target(pebble::math::vec2 target, float stiffness = 180.0f,
                                              float damping = 12.0f, float duration = 0.5f) const {
    ...
    template <typename SplineT>
    inline FollowPathBuilder<SplineT> follow_path(pebble::math::vec2& prop, SplineT spline,
