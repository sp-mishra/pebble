...
namespace ga {
    ...
    struct NeumannBC {
        static constexpr bool is_neumann = true;
    };

    struct WrapBC {
        static constexpr bool is_wrap = true;
    };

    struct ClampBC {
        static constexpr bool is_clamp = true;
    };
        ...
        inline std::size_t clamp_idx(std::ptrdiff_t i, std::size_t n) {
            if constexpr (requires { BC::is_wrap; }) {
                if constexpr (BC::is_wrap)
                    return static_cast<std::size_t>((i + static_cast<std::ptrdiff_t>(n)) % static_cast<std::ptrdiff_t>(
                        n));
            }
        ...
        T grid_get(const Grid2D<T, SP, CP>& g, std::ptrdiff_t r, std::ptrdiff_t c) {
            if constexpr (requires { BC::is_dirichlet; }) {
                if (r < 0 || c < 0 ||
                    ...
                    static_cast<std::size_t>(c) >= g.cols())
                    return T{0};
            }
        ...
        for (std::size_t i = 0; i < R; ++i)
            for (std::size_t j = 0; j < C; ++j) {
                ...
                for (std::size_t k = 0; k < kx.size(); ++k)
                    s += kx[k] * detail::grid_get<T, SP, CP, BC>(
                        u, i, static_cast<std::ptrdiff_t>(j) + static_cast<std::ptrdiff_t>(k) - hx);
                tmp.at(i, j) = s;
            }
        // vertical pass
        for (std::size_t i = 0; i < R; ++i)
            for (std::size_t j = 0; j < C; ++j) {
                ...
                for (std::size_t k = 0; k < ky.size(); ++k)
                    s += ky[k] * detail::grid_get<T, SP, CP, BC>(
                        tmp, static_cast<std::ptrdiff_t>(i) + static_cast<std::ptrdiff_t>(k) - hy, j);
                out.at(i, j) = s;
            }
        return out;
    }
    ...
    namespace detail {
        ...
        inline T weno5_flux(T vm2, T vm1, T v0, T vp1, T vp2) {
            ...
            T beta0 = (T{13.0 / 12.0}) * (vm2 - 2 * vm1 + v0) * (vm2 - 2 * vm1 + v0) + T{0.25} * (vm2 - 4 * vm1 + 3 *
                v0) * (vm2 - 4 * vm1 + 3 * v0);
            T beta1 = (T{13.0 / 12.0}) * (vm1 - 2 * v0 + vp1) * (vm1 - 2 * v0 + vp1) + T{0.25} * (vm1 - vp1) * (vm1 -
                vp1);
            T beta2 = (T{13.0 / 12.0}) * (v0 - 2 * vp1 + vp2) * (v0 - 2 * vp1 + vp2) + T{0.25} * (3 * v0 - 4 * vp1 +
                vp2) * (3 * v0 - 4 * vp1 + vp2);
            constexpr T eps = T{1e-6};
        ...
        for (std::size_t i = 0; i < R; ++i)
            for (std::size_t j = 0; j < C; ++j) {
                auto gi = [&](std::ptrdiff_t di, std::ptrdiff_t dj) {
                    return detail::grid_get<T, SP, CP, BC>(f, static_cast<std::ptrdiff_t>(i) + di,
