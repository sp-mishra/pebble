...
namespace ga {
    ...
    [[nodiscard]] EigenResult<T,SP,CP> eig_sym(
        ...
        auto off_diag_norm = [&]() {
            ...
            for (std::size_t i = 0; i < N; ++i)
                for (std::size_t j = i+1; j < N; ++j) { T v=S(i,j); s += v*v; }
            return s;
        };
    ...
    [[nodiscard]] LanczosResult<T> lanczos_eig(
        ...
        std::vector<Vector<T,SP,CP>> V; V.reserve(m+1);
        ...
        // simple unit v0
        T nrm0 = T{0}; for (std::size_t i=0;i<n;++i) nrm0+=v0[i]*v0[i]; nrm0=std::sqrt(nrm0);
        ...
        for (std::size_t j = 0; j < m; ++j) {
            ...
            for (std::size_t kk=0;kk<=j;++kk) {
                T dot2=T{0}; for(std::size_t i=0;i<n;++i) dot2+=V[kk][i]*w[i];
                for(std::size_t i=0;i<n;++i) w[i]-=dot2*V[kk][i];
            }
            T bnew = T{0}; for (std::size_t i=0;i<n;++i) bnew+=w[i]*w[i]; bnew=std::sqrt(bnew);
    ...
    [[nodiscard]] LanczosResult<T> arnoldi_eig(
        ...
        for (std::size_t j = 0; j < m && j < n-1; ++j) {
            ...
            T nrm = T{0}; for (std::size_t l=0;l<n;++l) nrm+=w[l]*w[l]; nrm=std::sqrt(nrm);
            if (nrm < T{1e-300}) break;
