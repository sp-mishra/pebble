...
namespace pebble::math {
    ...
    constexpr T cross(const ts::static_tensor<T, ts::default_storage_policy, ts::default_computation_policy, 2>& a,
                      const ts::static_tensor<T, ts::default_storage_policy, ts::default_computation_policy, 2>& b)
        noexcept {
    ...
    template <typename T>
    constexpr auto perp(
        const ts::static_tensor<T, ts::default_storage_policy, ts::default_computation_policy, 2>& v) noexcept {
    ...
    constexpr auto catmull_rom(
        const ts::static_tensor<T, ts::default_storage_policy, ts::default_computation_policy, N>& p0,
    ...
    template <size_t N>
    inline auto reflect(
        const ts::static_tensor<float, ts::default_storage_policy, ts::default_computation_policy, N>& I,
    ...
    struct aabb2 {
        ...
        constexpr void expand(const aabb2& o) noexcept {
            expand(o.lo);
            expand(o.hi);
        }
    ...
    struct aabb3 {
        ...
        constexpr void expand(const aabb3& o) noexcept {
            expand(o.lo);
            expand(o.hi);
        }
