...
namespace containers::spatial {
    ...
    public:
        ...
        explicit BarnesHutTree(BoundingBox2D bounds = BoundingBox2D{
    ...
    private:
        ...
        void insert_recursive(std::uint32_t node_idx, std::uint32_t body_idx,
            ...
            if (new_mass > 0.0f) {
                node.center_of_mass = (node.center_of_mass * node.total_mass + body.pos * body.mass) * (1.0f /
                    new_mass);
    ...
    inline void compute_all_forces(const BarnesHutTree& tree,
        ...
        // Multi-threaded parallel force sweep via persistent Pravaha thread pool
        if (n >= 64) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            out_forces[i] = tree.compute_force(bodies[i].pos, bodies[i].mass, static_cast<std::uint32_t>(i), bodies,
                                               policy);
        }
    }
} // namespace containers::spatial
