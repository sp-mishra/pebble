    ...
    struct PhotoshopBlendCombiner {
        ...
        [[nodiscard]] Color combine(const Color& dst, const Color& src, float opacity) const noexcept {
            ...
            auto blend_ch = [&](float cb, float cs) -> float {
                ...
                case BlendMode::Overlay: return
                        cb < 0.5f ? (2.0f * cb * cs) : (1.0f - 2.0f * (1.0f - cb) * (1.0f - cs));
                ...
                case BlendMode::HardLight: return cs < 0.5f
                                                      ? (2.0f * cb * cs)
                                                      : (1.0f - 2.0f * (1.0f - cb) * (1.0f - cs));
                case BlendMode::SoftLight: return cs < 0.5f
                                                      ? (cb - (1.0f - 2.0f * cs) * cb * (1.0f - cb))
                                                      : (cb + (2.0f * cs - 1.0f) * (std::sqrt(cb) - cb));
