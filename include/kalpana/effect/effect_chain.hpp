...
namespace kalpana {
    ...
    public:
        ...
        // Spectral specific
        [[nodiscard]] static EffectNode
        spectral_tint(spectral::SpectralColor pigment, float intensity = 1.0f) noexcept {
        ...
        [[nodiscard]] static EffectNode dof(pebble::math::vec2 focus_point = {0.0f, 0.0f},
            ...
            return EffectNode(DepthOfFieldEffect{
    ...
    [[nodiscard]] inline EffectNode shadow(float blur_r = 4.0f, pebble::math::vec2 offset = {2.0f, 2.0f},
                                           Color color = {0.0f, 0.0f, 0.0f, 0.5f}) noexcept {
        return EffectNode::shadow(blur_r, offset, color);
    }

    [[nodiscard]] inline EffectNode tint(Color color, float intensity = 1.0f) noexcept {
        return EffectNode::tint(color, intensity);
    }

    [[nodiscard]] inline EffectNode glow(float radius = 3.0f, Color color = colors::white()) noexcept {
        return EffectNode::glow(radius, color);
    }

    [[nodiscard]] inline EffectNode inner_glow(float radius = 2.0f, Color color = colors::white()) noexcept {
        return EffectNode::inner_glow(radius, color);
    }

    [[nodiscard]] inline EffectNode noise_effect(float amount = 0.1f, float scale = 1.0f) noexcept {
        return EffectNode::noise(amount, scale);
    }
    ...
    [[nodiscard]] inline EffectNode bloom(float threshold = 0.8f, float intensity = 1.0f) noexcept {
        return EffectNode::bloom(threshold, intensity);
    }
    ...
    [[nodiscard]] inline EffectNode hue_rotate(float degrees = 0.0f) noexcept {
        return EffectNode::hue_rotate(degrees);
    }
    ...
    [[nodiscard]] inline EffectNode vignette(float radius = 0.8f, float softness = 0.3f) noexcept {
        return EffectNode::vignette(radius, softness);
    }

    [[nodiscard]] inline EffectNode chromatic_aberration(float offset = 2.0f) noexcept {
        return EffectNode::chromatic_aberration(offset);
    }

    [[nodiscard]] inline EffectNode rounded_shadow(float corner_radius = 12.0f, float shadow_blur = 8.0f) noexcept {
        return EffectNode::rounded_shadow(corner_radius, shadow_blur);
    }

    [[nodiscard]] inline EffectNode spectral_tint(spectral::SpectralColor pigment, float intensity = 1.0f) noexcept {
        return EffectNode::spectral_tint(pigment, intensity);
    }

    [[nodiscard]] inline EffectNode spectral_bloom(float threshold = 0.8f) noexcept {
        return EffectNode::spectral_bloom(threshold);
    }

    [[nodiscard]] inline EffectNode pigment_granulation(float scale = 1.0f, float amount = 0.3f) noexcept {
        return EffectNode::pigment_granulation(scale, amount);
    }

    [[nodiscard]] inline EffectNode edge_darkening(float width = 2.0f, float intensity = 0.5f) noexcept {
        return EffectNode::edge_darkening(width, intensity);
    }

    [[nodiscard]] inline EffectNode backdrop_blur(float radius = 20.0f) noexcept {
        return EffectNode::backdrop_blur(radius);
    }
