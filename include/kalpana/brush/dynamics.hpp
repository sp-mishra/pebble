...
struct DynamicsBinding {
    ...
    [[nodiscard]] float evaluate(const BrushInputState& input) const noexcept {
        ...
        float raw = 0.0f;
        switch (source) {
            case DynamicsSource::Constant:  raw = base; break;
            case DynamicsSource::Pressure:  raw = input.pressure; break;
            case DynamicsSource::Velocity:  raw = input.velocity; break;
            case DynamicsSource::Tilt:      raw = input.tilt; break;
            case DynamicsSource::Direction: raw = input.direction; break;
            case DynamicsSource::Distance:  raw = std::fmod(input.distance * 0.01f, 1.0f); break;
            case DynamicsSource::Time:      raw = std::fmod(input.time, 1.0f); break;
            case DynamicsSource::Random:    raw = input.random; break;
        }
