...
public:
    ...
    [[nodiscard]] static BasicPathModifier warp(float amount, float frequency = 1.0f) {
        return BasicPathModifier([amount, frequency](const Path& p) { return path_ops::warp(p, amount, frequency); });
    }
...
[[nodiscard]] inline PathModifier roughen(float amount, float scale = 1.0f) { return PathModifier::roughen(amount, scale); }
...
[[nodiscard]] inline PathModifier warp(float amount, float frequency = 1.0f) { return PathModifier::warp(amount, frequency); }
