...
using Plot = std::variant<LinePlot, AreaPlot, StepPlot, StemPlot, ScatterPlot, ErrorBarPlot, BarPlot, HistogramPlot, GraphPlot, BubblePlot, PiePlot, HeatmapPlot>;
        ...
        if (!grid_enabled_) {
            ...
            for (std::size_t r = 0; r < rows_; ++r) {
                for (std::size_t c = 0; c < cols_; ++c) {
                    ...
                    render_cell(backend, cell_rect, cells_[idx].axes, cells_[idx].plots, cells_[idx].annotations, local);
                }
            }
        }
...
private:
    ...
    void draw_legend(Backend& backend, const Rect& rect, const std::vector<Plot>& plots,
        ...
        if ((legend_auto_ || legend_pos_ == LegendPosition::Auto) && !plots.empty()) {
            ...
            pos = (min_i == 0) ? LegendPosition::TopRight :
                  (min_i == 1) ? LegendPosition::TopLeft :
                  (min_i == 2) ? LegendPosition::BottomRight :
        ...
        if (pos == LegendPosition::TopLeft) {
            x = rect.x + 10.0f; y = rect.y + 12.0f;
        } else if (pos == LegendPosition::BottomRight) {
            x = rect.x + rect.w - 160.0f; y = rect.y + rect.h - h - 12.0f;
        } else if (pos == LegendPosition::BottomLeft) {
            x = rect.x + 10.0f; y = rect.y + rect.h - h - 12.0f;
        }
    ...
    [[nodiscard]] static std::pair<Range, Range> infer_ranges_for(const std::vector<Plot>& plots) {
        ...
        if (xr.degenerate()) { xr.min -= 1.0f; xr.max += 1.0f; }
        if (yr.degenerate()) { yr.min -= 1.0f; yr.max += 1.0f; }
        return {xr, yr};
    }
    ...
    static void draw_plot(Backend& backend, const ScatterPlot& plot,
                          ...
                          const Rect&, const Range&, const Range&) {
        for (const auto& p : plot.series.points()) {
            backend.draw_circle(xs.map(p.x), ys.map(p.y), plot.series.marker_style().radius, plot.series.marker_style().color);
        }
    }
    ...
    static void draw_plot(Backend& backend, const BarPlot& plot,
        ...
        for (const auto& p : plot.series.points()) {
            ...
            backend.draw_rect(x - 0.5f * w, top, w, std::max<Scalar>(1.0f, h), plot.series.stroke_style().color.with_alpha(0.6f));
        }
    }
    ...
    static void draw_plot(Backend& backend, const HistogramPlot& plot,
        ...
        for (const Scalar v : plot.values) {
            ...
            const std::size_t idx = std::min<std::size_t>(plot.bins - 1,
                static_cast<std::size_t>(t * static_cast<Scalar>(plot.bins)));
            ++counts[idx];
        }
        ...
        for (std::size_t i = 0; i < plot.bins; ++i) {
            ...
            backend.draw_rect(sx0, top, std::max<Scalar>(1.0f, sx1 - sx0), std::abs(sy0 - sy), plot.stroke.color.with_alpha(0.5f));
        }
    }
