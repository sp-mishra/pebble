            ...
            for (std::uint32_t iter = 0; iter < config_.iterations; ++iter) {
                ...
                for (std::size_t i = 0; i < graph.nodes.size(); ++i) {
                    for (std::size_t j = i + 1; j < graph.nodes.size(); ++j) {
                        ...
                        const Scalar dist2 = std::max(akruti::length_sq(d), policy_.min_distance);
                ...
                for (const auto& e : graph.edges) {
                    ...
                    const Scalar dist = std::max(akruti::length(d), policy_.min_distance);
                    const Vec2 dir = d / dist;
                ...
                for (std::size_t i = 0; i < graph.nodes.size(); ++i) {
                    ...
                    const Scalar speed = std::max(akruti::length(velocity[i]), policy_.min_distance);
