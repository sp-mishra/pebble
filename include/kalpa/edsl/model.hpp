...
namespace kalpa::edsl {
    ...
    [[nodiscard]] inline VarPack vars(std::size_t = 0)
    noexcept
 { return VarPack{}; }
    ...
    namespace detail {
        ...
        template <typename S, typename Coord>
        [[nodiscard]] S eval_term(const ::vakya::expr<Var>& e, const Coord& coord) {
            return eval_term<S>(e.value, coord);
        }

        template <typename S, typename T, typename Coord>
        [[nodiscard]] S eval_term(const ::vakya::expr<Const<T>>& e, const Coord& coord) {
            return eval_term<S>(e.value, coord);
        }

        template <typename S, typename Coord>
        [[nodiscard]] S eval_term(const ::vakya::expr_ref<Var>& e, const Coord& coord) {
            return eval_term<S>(*e.p, coord);
        }

        template <typename S, typename T, typename Coord>
        [[nodiscard]] S eval_term(const ::vakya::expr_ref<Const<T>>& e, const Coord& coord) {
            return eval_term<S>(*e.p, coord);
        }
    ...
    namespace detail {
        ...
        [[nodiscard]] constexpr Relation relation_of() {
            if constexpr (std::is_same_v<Tag, ::vakya::eq_tag>) return Relation::Eq;
                ...
                static_assert(std::is_same_v<Tag, ::vakya::eq_tag>,
                              "kalpa::edsl::subject_to: argument is not a ==, <= or >= comparison");
                return Relation::Eq;
            }
        }
    } // namespace detail
    ...
    struct Analysis {
        bool linear{true}, convex{true}, smooth{true};
        std::size_t dim{0};
    };
    ...
    namespace detail {
        ...
        void analyze_walk(const Node& node, Analysis& a) {
            ...
            if constexpr (std::is_same_v<Tag, ::vakya::add_tag> ||
                ...
                analyze_child(std::get < 0 > (ch), a);
                // convexity flips but − of convex handled by callers; keep smooth/linear
            }
            else if constexpr (std::is_same_v<Tag, ::vakya::mul_tag>) {
                ...
                a.linear = false;
                a.convex = false; // general 1/g is nonconvex
                analyze_child(std::get < 0 > (ch), a);
                analyze_child(std::get < 1 > (ch), a);
            }
            else {
