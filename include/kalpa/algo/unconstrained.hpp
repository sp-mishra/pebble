...
namespace kalpa {
    ...
    struct ConjugateGradient {
        ...
        void reset(std::size_t n) {
            d_prev = ga::Vector<T>(n, T{0});
            g_prev = ga::Vector<T>(n, T{0});
            first = true;
        }
    ...
    struct BFGS {
        ...
        void update(const ga::Vector<T>& x0, const ga::Vector<T>& g0,
            ...
            for (std::size_t i = 0; i < n; ++i) {
                T a{};
                for (std::size_t j = 0; j < n; ++j) a += H(i, j) * yv[j];
                Hy[i] = a;
            }
