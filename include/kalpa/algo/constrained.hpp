...
namespace kalpa {
    ...
    struct Polytope {
        ...
        void project(const ga::Vector<T>& x, ga::Vector<T>& out) const {
            ...
            for (std::size_t r = 0; r < m; ++r) {
                T s{};
                for (std::size_t j = 0; j < n; ++j) s += A(r, j) * A(r, j);
                a2[r] = s;
            }
            for (std::size_t sweep = 0; sweep < sweeps; ++sweep) {
                ...
                for (std::size_t r = 0; r < m; ++r) {
                    ...
                    T ax{};
                    for (std::size_t j = 0; j < n; ++j) ax += A(r, j) * out[j];
                    const T viol = ax - b[r];
                    if (viol > T{0}) {
                        const T t = viol / a2[r];
        ...
        solve(const ga::Matrix<T>& A, const ga::Vector<T>& b,
            ...
            for (std::size_t iter = 0; iter < max_iter; ++iter) {
                ...
                for (std::size_t j = 0; j < n; ++j) {
                    ...
                    T yA{};
                    for (std::size_t i = 0; i < m; ++i) yA += y[i] * Aa(i, j);
                    if (cc[j] - yA < -tol) {
                        enter = j;
                        break;
                    } // Bland's rule
    ...
    private:
        ...
        static T big_m(const ga::Vector<T>& c) {
            T mx{};
            for (std::size_t i = 0; i < c.size(); ++i) mx = std::max(mx, std::abs(c[i]));
        ...
        assemble(const std::vector<std::size_t>& basis, const ga::Vector<T>& xB,
            ...
            Result<T> r;
            r.x = ga::Vector<T>(nvar, T{0});
            for (std::size_t i = 0; i < m; ++i)
                if (basis[i] < nvar) r.x[basis[i]] = xB[i];
            T obj{};
            for (std::size_t j = 0; j < nvar; ++j) obj += cc[j] * r.x[j];
            r.f = obj;
            r.status = Status::Converged;
            return r;
        }
    ...
    private:
        ...
        static ExactSimplexResult finish(const std::vector<std::vector<Fraction>>& Tb,
            ...
            ExactSimplexResult r;
            for (std::size_t i = 0; i < m; ++i)
                if (basis[i] >= nvar && Tb[i][n].positive()) {
                    r.cause = Cause::Infeasible;
                    return r;
                }
    ...
    struct InteriorPoint {
        ...
        solve(const ga::Matrix<T>& H, const ga::Vector<T>& c,
            ...
            for (std::size_t it = 0; it < max_iter; ++it) {
                ...
                T mu{};
                for (std::size_t i = 0; i < n; ++i) mu += x[i] * z[i];
                mu /= static_cast<T>(n);
            ...
            if (H.rows() == n)
                for (std::size_t i = 0; i < n; ++i) {
                    T hx{};
                    for (std::size_t j = 0; j < n; ++j) hx += H(i, j) * x[j];
                    obj += T{0.5} * x[i] * hx;
                }
    ...
    struct SQP {
        ...
        solve(const F& f, const ga::Vector<T>& x0, const Deriv& deriv, const ConSet& cons) const {
            ...
            auto c1norm = [&](const ga::Vector<T>& cv) {
                T s{};
                for (std::size_t i = 0; i < cv.size(); ++i) s += std::abs(cv[i]);
                return s;
            };
            ...
            for (std::size_t it = 0; it < max_iter; ++it) {
                ...
                ga::Matrix<T> W(n, n, T{0});
                {
                    ...
                    for (std::size_t j = 0; j < n; ++j) {
                        e[j] = T{1};
                        deriv.hessian_vec(f, x, e, col);
                        for (std::size_t i = 0; i < n; ++i) W(i, j) = col[i];
                        e[j] = T{0};
                    }
                    for (std::size_t i = 0; i < n; ++i)
                        for (std::size_t j = i + 1; j < n; ++j) {
                            T a = T{0.5} * (W(i, j) + W(j, i));
                            W(i, j) = a;
                            W(j, i) = a;
                        }
                }
    ...
    struct SQP_Ineq {
        ...
        solve(const F& f, const ga::Vector<T>& x0, const Deriv& deriv,
            ...
            auto viol1 = [&](const ga::Vector<T>& cv) { // Σ max(0, cᵢ)
                T s{};
                for (std::size_t i = 0; i < cv.size(); ++i) s += std::max(T{0}, cv[i]);
                return s;
            };
            ...
            for (std::size_t it = 0; it < max_iter; ++it) {
                ...
                for (std::size_t inner = 0; inner < inner_iter; ++inner) {
                    ...
                    std::size_t drop = mi;
                    T most_neg = -active_tol;
                    for (std::size_t i = 0; i < mi; ++i) {
                        if (active[i] && mu[i] < most_neg) {
                            most_neg = mu[i];
                            drop = i;
                        }
                    }
                ...
                T compl_gap{};
                for (std::size_t i = 0; i < mi; ++i) compl_gap += mu[i] * ci[i];
                T dual_inf{};
                for (std::size_t i = 0; i < mi; ++i) dual_inf += std::min(T{0}, mu[i]) * std::min(T{0}, mu[i]);
    ...
    private:
        ...
        static void powell_bfgs_update(ga::Matrix<T>& W, const ga::Vector<T>& s,
            ...
            T sr{};
            for (std::size_t j = 0; j < n; ++j) sr += s[j] * rr[j];
