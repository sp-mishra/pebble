    ...
    [[nodiscard]] inline auto make_ffi_task(
        ...
        if (!proxy.valid() || arguments.size() != proxy.arity || proxy.arity > 8) {
            return task(std::string{name}, std::function < Outcome<runtime_value>() >
            {
                [proxy, count = arguments.size()]() -> Outcome<runtime_value> {
                    const auto message = !proxy.valid()
                                             ? "invalid native_proxy"
                                             : proxy.arity > 8
                                             ? "arity > 8"
                                             : "arity mismatch: expected " + std::to_string(proxy.arity) + " got " +
                                             std::to_string(count);
        ...
        return task(std::string{name}, std::function < Outcome<runtime_value>() >
        {
            [proxy, captured = std::move(captured)]() mutable -> Outcome<runtime_value> {
                ...
                std::int64_t result{};
                switch (proxy.arity) {
                case 0: result = call_native<0>(proxy.fn_ptr, native_arguments);
                    break;
                case 1: result = call_native<1>(proxy.fn_ptr, native_arguments);
                    break;
                case 2: result = call_native<2>(proxy.fn_ptr, native_arguments);
                    break;
                case 3: result = call_native<3>(proxy.fn_ptr, native_arguments);
                    break;
                case 4: result = call_native<4>(proxy.fn_ptr, native_arguments);
                    break;
                case 5: result = call_native<5>(proxy.fn_ptr, native_arguments);
                    break;
                case 6: result = call_native<6>(proxy.fn_ptr, native_arguments);
                    break;
                case 7: result = call_native<7>(proxy.fn_ptr, native_arguments);
                    break;
                case 8: result = call_native<8>(proxy.fn_ptr, native_arguments);
                    break;
