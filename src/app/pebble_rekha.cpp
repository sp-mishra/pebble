...
namespace {
    ...
    rekha::Figure build_figure(const RekhaApp& app) {
        ...
        if (app.panel_graph) {
            ...
            fig.add(rekha::ScatterPlot{
                rekha::XYSeries{"off"}.add(50.0f, 50.0f).marker({kalpana::colors::white(), 2.0f})
            });
        }
...
sapp_desc sokol_main(int /*argc*/, char** /*argv*/) {
    ...
    d.height = H;
    d.window_title =
        "Rekha Dashboard [1..4 toggle panels | 0 all | L legend | A auto-legend | R reset graph | ESC quit]";
