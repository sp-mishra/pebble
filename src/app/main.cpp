...
static kalpana::Path body_shape_path(const SimBody& b, float sx, float sy) {
    ...
    switch (b.kind) {
    case ShapeKind::Circle: p.circle(x, y, s);
        break;
    case ShapeKind::Box: {
...
static void init_prakriti_continuum() {
    ...
    for (int i = 0; i < 10; ++i) {
        app.prakriti_world->particles().add({
            .position = pebble::math::vec2(ARENA_W - 140.0f + randf(-20.0f, 20.0f),
                                           ARENA_Y0 + 120.0f + randf(-15.0f, 15.0f)),
...
static void step_liquids(float dt) {
    ...
    for (auto& pool : app.liquids) {
        ...
        for (auto& d : pool.drops) {
            ...
            if (d.pos[0] < r) {
                d.pos[0] = r;
                d.vel[0] = std::abs(d.vel[0]) * 0.45f;
            }
            if (d.pos[0] > ARENA_W - r) {
                d.pos[0] = ARENA_W - r;
                d.vel[0] = -std::abs(d.vel[0]) * 0.45f;
            }
            if (d.pos[1] < ARENA_Y0 + r) {
                d.pos[1] = ARENA_Y0 + r;
                d.vel[1] = std::abs(d.vel[1]) * 0.45f;
            }
            if (d.pos[1] > ARENA_H - r) {
                d.pos[1] = ARENA_H - r;
                d.vel[1] = -std::abs(d.vel[1]) * 0.32f;
            }
...
using AkrutiShapeVar = std::variant<akruti::Circle, akruti::Box, akruti::OrientedBox, akruti::Capsule, akruti::Triangle,
                                    akruti::RoundedBox, akruti::Sector, akruti::Segment, akruti::ConvexPoly < kPolyMax>
,
akruti::RoundedPoly<kPolyMax>
>;
...
static akruti::Manifold test_body_collision(const SimBody& a, const SimBody& b) {
    ...
    return std::visit([&](const auto& shape_a) -> akruti::Manifold {
        return std::visit([&](const auto& shape_b) -> akruti::Manifold {
            ...
            else if constexpr (std::is_same_v<TypeA, akruti::OrientedBox>&& std::is_same_v<
                TypeB, akruti::OrientedBox>) {
...
static void step_sim_bodies(float dt) {
    ...
    for (int step = 0; step < kSubsteps; ++step) {
        ...
        for (auto& b : app.bodies) {
            ...
            if (b.pos[0] < r) {
                b.pos[0] = r;
                b.vel[0] = std::abs(b.vel[0]);
            }
            if (b.pos[0] > ARENA_W - r) {
                b.pos[0] = ARENA_W - r;
                b.vel[0] = -std::abs(b.vel[0]);
            }
            if (b.pos[1] < ARENA_Y0 + r) {
                b.pos[1] = ARENA_Y0 + r;
                b.vel[1] = std::abs(b.vel[1]);
            }
            if (b.pos[1] > ARENA_H - r) {
                b.pos[1] = ARENA_H - r;
                b.vel[1] = -std::abs(b.vel[1]);
            }
        }
                        ...
                        if (step == 0) {
                            ...
                            if (impact > 170.0f) {
                                for (auto* body_ptr : std::array<SimBody*, 2>{&a, &b}) {
                                    ...
                                    bool brittle = mat->phase_fractions.solid() > 0.7f && mat->params.ultimate_strain <
                                        0.01f;
    ...
    if (app.prakriti_world) {
        ...
        for (prakriti::Index i = 0; i < P.size(); ++i) {
            ...
            if (P.f_gas[i] > 0.4f) {
                ...
                pcol = kalpana::spectral::mix(kalpana::Color{0.0f, 0.75f, 1.0f, 0.85f},
                                              kalpana::Color{1.0f, 0.4f, 0.1f, 0.9f}, heat_frac);
                pr = 5.5f;
            }
            else {
...

    {
        ...
        for (std::size_t i = 1; i < pts.size(); ++i) {
            ...
            scene.add(kalpana::Node::shape(seg,
                                           kalpana::Paint::stroke(kalpana::Color{1.0f, 0.2f, 0.9f, t * 0.9f},
                                                                  2.7f * t)));
        }
    }
...
static void init_cb() {
    ...
    app.pass_action.colors[0].clear_value = {0.025f, 0.028f, 0.06f, 1.0f};
...
sapp_desc sokol_main(int /*argc*/, char** /*argv*/) {
    ...
    d.height = H;
    d.window_title =
        "Pebble Multiphysics & Continuum Showcase (Prakriti, Akruti, Gati, Spandana, Kalpana) [R]/[SPC] reset";
