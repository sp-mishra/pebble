...
namespace {
    ...
    static void update_kmeans(MLState& s) {
        ...
        for (int i = 0; i < N; ++i) {
            X(size_t(i), size_t(0)) = s.km_x[i];
            X(size_t(i), size_t(1)) = s.km_y[i];
        }
        manas::ml::KMeans<manas::ml::KMeansPPInit> km(K);
        km.fit(X);
        s.km_labels = km.labels();
        const auto& c = km.cluster_centers();
        s.km_inertia = 0;
        for (int i = 0; i < N; ++i) {
            const int k = s.km_labels[i];
    ...
    static void update_regression(MLState& s) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            s.lr_y[i] = std::clamp(50.0f + 28.0f * std::sin(x * 0.065f + s.t * 0.45f)
                                   + 8.0f * std::sin(x * 0.14f + s.t * 0.3f) + 9.0f * (std::sin(
                                       static_cast<float>(i) * 1.73f) - 0.3f), 3.0f, 97.0f);
        }
        const float sc = 1.0f / 100.0f;
        ts::tensor<float> X({size_t(N), size_t(3)}), Y({size_t(N)});
        for (int i = 0; i < N; ++i) {
            const float x = s.lr_x[i] * sc;
            X(size_t(i), size_t(0)) = x;
            X(size_t(i), size_t(1)) = x * x;
            X(size_t(i), size_t(2)) = x * x * x;
            Y(size_t(i)) = s.lr_y[i] * sc;
        }
        manas::ml::LinearRegression<> lr;
        lr.fit(X, Y);
        const int M = 60;
        s.lr_pred_x.resize(M);
        s.lr_pred_y.resize(M);
        ts::tensor<float> Xp({size_t(M), size_t(3)});
        for (int i = 0; i < M; ++i) {
            const float x = static_cast<float>(i) / static_cast<float>(M - 1);
            Xp(size_t(i), size_t(0)) = x;
            Xp(size_t(i), size_t(1)) = x * x;
            Xp(size_t(i), size_t(2)) = x * x * x;
            s.lr_pred_x[i] = x * 100.0f;
        }
        const auto pred = lr.predict(Xp);
        for (int i = 0; i < M; ++i) s.lr_pred_y[i] = std::clamp(pred(std::vector<size_t>{size_t(i)}) * 100.0f, 0.0f,
                                                                100.0f);
        const auto tp = lr.predict(X);
        float ss_res = 0, ss_tot = 0, my = 0;
        for (int i = 0; i < N; ++i) my += s.lr_y[i];
        my /= N;
        s.lr_resid.resize(N);
        for (int i = 0; i < N; ++i) {
            const float yh = tp(std::vector<size_t>{size_t(i)}) * 100.0f;
            s.lr_resid[i] = s.lr_y[i] - yh;
            ss_res += s.lr_resid[i] * s.lr_resid[i];
            const float d = s.lr_y[i] - my;
            ss_tot += d * d;
        }
    ...
    static void init_nn(MLState& s) {
        ...
        s.nn_opt = Adam<>{0.005f};
        s.nn_loss_hist.clear();
        s.nn_epoch = 0;
        s.nn_init = true;
    }
    ...
    static void step_nn(MLState& s) {
        ...
        for (int i = 0; i < 8; ++i) {
            s.nn_loss_cur = manas::nn::train_step(s.nn_model, manas::nn::TensorVar(X), manas::nn::TensorVar(Y),
                                                  manas::nn::mse_loss, s.nn_opt);
            ++s.nn_epoch;
        }
        ...
        const float ty[4] = {0, 1, 1, 0};
        int c = 0;
        for (int i = 0; i < 4; ++i) if (std::abs(pt.data(std::vector<size_t>{size_t(i), size_t(0)}) - ty[i]) < 0.5f) ++
            c;
    ...
    static void update_solvers(MLState& s) {
        ...
        auto obj = [&](const ga::Vector<double>& x)-> double {
            const double dx = x[0] - tx, dy = x[1] - ty;
            return dx * dx + dy * dy;
        };
        const auto prob = kalpa::make_problem(obj);
        ga::Vector<double> x0(2);
        x0[0] = 50.0;
        x0[1] = 50.0;
        auto run = [&](int idx, auto algo) {
            using Algo = decltype(algo);
            using Deriv = kalpa::Derivatives<kalpa::FiniteDiff, double>;
            kalpa::Solver<Algo, Deriv> slv(std::move(algo));
            const auto res = slv.solve(prob, x0);
            const float fv = res
                                 ? static_cast<float>(res->f)
                                 : (s.solver_hist[idx].empty() ? 1000.f : s.solver_hist[idx].back());
            s.solver_best[idx] = std::min(s.solver_best[idx], fv);
            if (res) { s.solver_iters[idx] = res->iterations; }
            s.solver_hist[idx].push_back(fv);
            if (static_cast<int>(s.solver_hist[idx].size()) > MLState::KALPA_HIST) s.solver_hist[idx].erase(
                s.solver_hist[idx].begin());
            s.solver_spark[idx].push_back(fv);
            if (static_cast<int>(s.solver_spark[idx].size()) > MLState::SPARK) s.solver_spark[idx].erase(
                s.solver_spark[idx].begin());
        };
        run(0, kalpa::GradientDescent<double>{});
        run(1, kalpa::Momentum<double>{});
        run(2, kalpa::Nesterov<double>{});
        run(3, kalpa::Adam<double>{});
        run(4, kalpa::LBFGS<double>{});
    }
    ...
    static rekha::Figure make_kmeans_fig(const MLState& s) {
        ...
        for (int k = 0; k < K; ++k) {
            ...
            c.add(s.km_cx[k], s.km_cy[k]);
            fig.add(rekha::ScatterPlot{std::move(c)});
        }
        rekha::BubblePlot bp;
        bp.stroke = {kalpana::Color{1, 0.9f, 0.3f, 0.5f}, 0.8f};
        for (int k = 0; k < K; ++k) {
    ...
    static rekha::Figure make_regression_fig(const MLState& s) {
        const int N = MLState::LR_N;
        rekha::XYSeries obs("obs");
        obs.stroke({pal::k_accent, 0.0f}).marker({pal::k_accent, 3.0f});
        for (int i = 0; i < N; ++i) obs.add(s.lr_x[i], s.lr_y[i]);
        rekha::XYSeries fit("fit");
        fit.stroke({pal::k_orange, 2.2f}).marker({pal::k_orange, 0.0f});
        for (int i = 0; i < static_cast<int>(s.lr_pred_x.size()); ++i) fit.add(s.lr_pred_x[i], s.lr_pred_y[i]);
        rekha::XYSeries res("res");
        res.stroke({kalpana::Color{0.9f, 0.3f, 0.3f, 0.4f}, 0.8f}).marker({});
        for (int i = 0; i < N; ++i) {
            res.add(s.lr_x[i], s.lr_y[i]);
            res.add(s.lr_x[i], std::clamp(s.lr_y[i] - s.lr_resid[i], 0.0f, 100.0f));
        }
        rekha::Figure fig;
        fig.theme(rekha::Figure::theme_dark_neon())
           .axes(rekha::Axes{
               .x_label = "x", .y_label = "signal", .ticks = 4,
               .x_range_override = true, .y_range_override = true, .x_range = {0, 100}, .y_range = {0, 100}
           })
           .add(rekha::LinePlot{std::move(res)}).add(rekha::ScatterPlot{std::move(obs)}).add(rekha::LinePlot{
               std::move(fit)
           });
        return fig;
    }
    ...
    static rekha::Figure make_nn_fig(const MLState& s) {
        ...
        float mx = 1.0f;
        for (float v : s.nn_loss_hist) mx = std::max(mx, v);
        const float rng = mx + 1e-5f;
        rekha::XYSeries loss("loss");
        loss.stroke({pal::k_red, 2.2f});
        for (int i = 0; i < H2; ++i) loss.add(static_cast<float>(s.nn_epoch - H2 + i),
                                              s.nn_loss_hist[i] / rng * 100.0f);
        rekha::XYSeries sm("smooth");
        sm.stroke({pal::k_yellow, 1.2f});
        for (int i = 0; i < H2; ++i) {
            int lo = std::max(0, i - 8);
            float avg = 0;
            int cnt = 0;
            for (int j = lo; j <= i; ++j) {
                avg += s.nn_loss_hist[j];
                ++cnt;
            }
            avg /= cnt;
            sm.add(static_cast<float>(s.nn_epoch - H2 + i), avg / rng * 100.0f);
        }
        rekha::XYSeries acc("acc");
        acc.stroke({pal::k_green, 1.0f});
        if (H2 > 0) {
            acc.add(static_cast<float>(s.nn_epoch - H2), s.nn_acc * 100.0f);
            acc.add(static_cast<float>(s.nn_epoch), s.nn_acc * 100.0f);
        }
        rekha::XYSeries area("area");
        area.stroke({pal::k_red, 0.0f});
        for (int i = 0; i < H2; ++i) area.add(static_cast<float>(s.nn_epoch - H2 + i),
                                              s.nn_loss_hist[i] / rng * 100.0f);
        const float x0 = H2 > 0 ? static_cast<float>(s.nn_epoch - H2) : 0.0f, x1 = static_cast<float>(std::max(
                        1, s.nn_epoch));
        rekha::Figure fig;
        fig.theme(rekha::Figure::theme_dark_neon())
           .axes(rekha::Axes{
               .x_label = "epoch", .y_label = "loss", .ticks = 4,
    ...
    static rekha::Figure make_solver_fig(const MLState& s) {
        ...
        for (int si = 0; si < 5; ++si) {
            ...
            rekha::XYSeries pt(sn[si]);
            pt.stroke({pal::k_solver[si], 0.0f}).marker({pal::k_solver[si], 7.0f});
            const int H2 = static_cast<int>(s.solver_hist[si].size());
    ...
    struct PanelHeader : pebble::drishya::widgets::WidgetBase {
        ...
        void paint(P& painter, pebble::drishya::Rect2D box) const {
            static int dbg = 0;
            if (dbg++ < 4)
                std::fprintf(stderr, "PanelHeader[%s] box=%.1f,%.1f,%.1f,%.1f\n",
    ...
    struct LiveBadge : pebble::drishya::widgets::WidgetBase {
        ...
        void paint(P& painter, pebble::drishya::Rect2D box) const {
            ...
            const uint32_t c = ml->paused
                                   ? pal::orange
                                   : static_cast<uint32_t>(0xFF000000u | (uint32_t((0x10 + int(0x10 * p)) & 0xFF) << 16)
                                       | (0xB2u << 8) | uint32_t((0x59 + int(0x10 * p)) & 0xFF));
    ...
    struct LiveStatTile : pebble::drishya::widgets::WidgetBase {
        ...
        void paint(P& painter, pebble::drishya::Rect2D box) const {
            ...
            const float vals[] = {
                static_cast<float>(ml->nn_epoch), ml->nn_loss_cur, ml->nn_acc * 100.0f, ml->lr_r2, ml->km_inertia
            };
            ...
            char buf[24];
            std::snprintf(buf, sizeof(buf), fmts[stat_id], vals[stat_id]);
    ...
    struct SolverBottomCell : pebble::drishya::widgets::WidgetBase {
        ...
        void paint(P& painter, pebble::drishya::Rect2D box) const {
            ...
            char buf[24];
            std::snprintf(buf, sizeof(buf), "f=%.2f", ml->solver_best[idx] > 1e8f ? 0.0f : ml->solver_best[idx]);
            if constexpr (pebble::drishya::ColorPainter<P>) painter.set_color(pal::dim);
            painter.text(std::string_view{buf}, {box.x + 8.0f, box.y + 30.0f}, 9.0f);
            const auto& sp = ml->solver_spark[idx];
            if (sp.size() < 2) { return; }
            float lo = sp[0], hi = sp[0];
            for (float v : sp) {
                lo = v < lo ? v : lo;
                hi = v > hi ? v : hi;
            }
    ...
    struct LiveFigWidget : pebble::drishya::widgets::WidgetBase {
        ...
        void paint(P& painter, pebble::drishya::Rect2D box) const {
            static int dbg = 0;
            if (dbg++ < 8)
                std::fprintf(stderr, "LiveFigWidget paint fig=%p box=%.1f,%.1f,%.1f,%.1f\n",
    ...
    static void build_ui(MLDashApp& app) {
        ...
        const auto tbar = ui.add_child(root, [] {
            ...
            p.style_.height = SizeSpec::Px(38.0f);
            p.style_.align_items = Align::Center;
            p.style_.padding = Edges{10, 4, 8, 4};
            return p;
        }());
        ui.add_child(tbar, [] {
            pebble::drishya::widgets::Label l("PEBBLE INTELLIGENCE DASHBOARD", pal::accent, 14.0f);
            l.style_.flex_grow = 1.0f;
            return l;
        }());
        ...
        const auto content = ui.add_child(root, [] {
            ...
            s.grow(1.0f).align(Align::Stretch);
            return s;
        }());
        ...
        const auto sidebar = ui.add_child(content, [] {
            ...
            p.style_.width = SizeSpec::Px(178.0f);
            return p;
        }());
        ...
        const auto panels_col = ui.add_child(content, [] {
            ...
            s.grow(1.0f).align(Align::Stretch).padding(Edges{4, 4, 4, 4});
            return s;
        }());
        ...
        for (int row = 0; row < 2; ++row) {
            ...
            for (int col = 0; col < 2; ++col) {
                ...
                const auto p = ui.add_child(r, [&] {
                    ...
                    pan.style_.flex_grow = 1.0f;
                    pan.style_.width = SizeSpec::Fr(1.0f);
                    pan.style_.align_items = Align::Stretch;
                    pan.style_.margin = Edges{0, col < 1 ? 4.0f : 0.0f, 0, 0};
                    return pan;
                }());
        ...
        const auto bottom = ui.add_child(root, [] {
            ...
            p.style_.height = SizeSpec::Px(90.0f);
            p.style_.align_items = Align::Stretch;
            return p;
        }());
        for (int si = 0; si < 5; ++si)
    ...
    void init_cb() {
        ...
        {
            sg_buffer_desc d{};
            d.size = 768 * 1024 * sizeof(kalpana::sokol_backend::Vertex);
        ...
        {
            sg_buffer_desc d{};
            d.size = 1536 * 1024 * sizeof(uint32_t);
    ...
    void event_cb(const sapp_event* ev) {
        ...
        switch (ev->key_code) {
        case SAPP_KEYCODE_ESCAPE: sapp_quit();
            break;
        case SAPP_KEYCODE_SPACE: g_app.ml.paused ^= true;
            break;
        case SAPP_KEYCODE_R:
            g_app.ml.t = 0;
            g_app.ml.frame = 0;
            init_nn(g_app.ml);
            for (int i = 0; i < MLState::NUM_SOLVERS; ++i) {
                g_app.ml.solver_hist[i].clear();
                g_app.ml.solver_spark[i].clear();
                g_app.ml.solver_best[i] = 1e9f;
            }
            break;
        default: break;
        }
