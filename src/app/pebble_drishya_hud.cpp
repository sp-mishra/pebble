...
namespace {
    ...
    struct UICtx {
        ...
        void round_rect(float x, float y, float w, float h, float r, kalpana::Color c) {
            kalpana::Path p;
            p.round_rect(ox + x, oy + y, w, h, r, r);
            scene.add(kalpana::Node::shape(std::move(p), kalpana::Paint::fill(c)));
        }
        ...
        void bar(float x, float y, float w, float h, float f, kalpana::Color bg_c, kalpana::Color fg_c,
                 float r = 3.0f) {
    ...
    static void draw_left_hud(UICtx& ui, const HUDState& s) {
        ...
        char hbuf[8];
        std::snprintf(hbuf, sizeof(hbuf), "%.0f%%", s.health * 100.0f);
        ...
        char ebuf[8];
        std::snprintf(ebuf, sizeof(ebuf), "%.0f%%", s.energy * 100.0f);
        ...
        char sbuf[10];
        std::snprintf(sbuf, sizeof(sbuf), "%.0f%%", s.shield * 100.0f);
    ...
    static void draw_ability_bar(UICtx& ui, const HUDState& s) {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            if (!ready) {
                ...
                char cbuf[8];
                std::snprintf(cbuf, sizeof(cbuf), "%.1f", rem);
    ...
    void frame_cb() {
        ...
        // ── Threat map (right panel) ─────────────────────────────────────────────
        {
            ...
            kalpana::Path bg;
            bg.round_rect(ui.ox + rx, ui.oy + ry, rw, rh, 6.0f, 6.0f);
