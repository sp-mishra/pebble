...
TEST_CASE("CG: 1D Laplacian 5×5 converges", "[iterative][cg]") {
...
TEST_CASE("CG: with Jacobi preconditioner", "[iterative][cg][jacobi]") {
...
TEST_CASE("BiCGSTAB: non-symmetric 4×4", "[iterative][bicgstab]") {
...
TEST_CASE("GMRES: 4×4 system", "[iterative][gmres]") {
    Matrix<double> A(4, 4, 0.0);
...
TEST_CASE("MINRES: symmetric indefinite 4×4", "[iterative][minres]") {
    Matrix<double> A(4, 4, 0.0);
...
TEST_CASE("Jacobi sweep: smoother on Laplacian", "[iterative][jacobi][relaxation]") {
...
TEST_CASE("Gauss-Seidel sweep: better than Jacobi", "[iterative][gs][relaxation]") {
    constexpr std::size_t N = 4;
    auto A = make_spd(N);
    Vector<double> b(N, 1.0);
    Vector<double> x(N, 0.0);
...
TEST_CASE("IterResult: has expected fields", "[iterative][result]") {
    IterResult<float> r;
    r.converged = true;
    r.iterations = 10;
