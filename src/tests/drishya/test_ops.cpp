...
TEST_CASE("ops: visit covers all nodes pre-order", "[drishya][ops]") {
    Built x = make_tree();
    std::size_t count = 0;
...
TEST_CASE("ops: subtree_size counts inclusive", "[drishya][ops]") {
    Built x = make_tree();
...
TEST_CASE("ops: depth_of measures from root", "[drishya][ops]") {
    Built x = make_tree();
...
TEST_CASE("ops: find returns first matching node", "[drishya][ops]") {
    Built x = make_tree();
...
TEST_CASE("ops: find_all fills the output span heap-free", "[drishya][ops]") {
    Built x = make_tree();
