...
TEST_CASE("static_parse_output: has success field", "[samasa][static_parse]") {
    using Out = static_parse_output<SK, TK, 256, 256>;
    STATIC_REQUIRE(std::is_same_v<decltype(std::declval<Out>().success), bool>);
}

TEST_CASE("static_parse_output: has token_count field", "[samasa][static_parse]") {
    using Out = static_parse_output<SK, TK, 256, 256>;
    STATIC_REQUIRE(std::is_same_v<decltype(std::declval<Out>().token_count), std::uint32_t>);
}

TEST_CASE("static_parse_output: has event_count field", "[samasa][static_parse]") {
    using Out = static_parse_output<SK, TK, 256, 256>;
    STATIC_REQUIRE(std::is_same_v<decltype(std::declval<Out>().event_count), std::uint32_t>);
}

TEST_CASE("static_parse_output: has diag_count field", "[samasa][static_parse]") {
    using Out = static_parse_output<SK, TK, 256, 256>;
    STATIC_REQUIRE(std::is_same_v<decltype(std::declval<Out>().diag_count), std::uint32_t>);
}

TEST_CASE("static_parse_output: tokens array size matches template parameter", "[samasa][static_parse]") {
    using Out = static_parse_output<SK, TK, 16, 8>;
    STATIC_REQUIRE(std::tuple_size_v<decltype(std::declval<Out>().tokens)> == 16);
}

TEST_CASE("static_parse_output: events array size matches template parameter", "[samasa][static_parse]") {
    using Out = static_parse_output<SK, TK, 16, 8>;
...
TEST_CASE("parse_static: runs grammar at compile time — valid input succeeds", "[samasa][static_parse]") {
...
TEST_CASE("parse_static: return type is static_parse_output<SK,TK>", "[samasa][static_parse]") {
...
TEST_CASE("parse_output: has success field", "[samasa][static_parse]") {
    using Out = parse_output<SK, TK>;
    STATIC_REQUIRE(std::is_same_v<decltype(std::declval<Out>().success), bool>);
}

TEST_CASE("parse_output: has tokens field of type token_buffer<TK>", "[samasa][static_parse]") {
    using Out = parse_output<SK, TK>;
    STATIC_REQUIRE(std::is_same_v<decltype(std::declval<Out>().tokens), token_buffer<TK>>);
}

TEST_CASE("parse_output: has diagnostics field of type collecting_sink<diagnostic>", "[samasa][static_parse]") {
    using Out = parse_output<SK, TK>;
    using Expected = lang::collecting_sink<diagnostic>;
    STATIC_REQUIRE(std::is_same_v<decltype(std::declval<Out>().diagnostics), Expected>);
}

TEST_CASE("parse_output: has stats field of type parse_tree_stats", "[samasa][static_parse]") {
    using Out = parse_output<SK, TK>;
    STATIC_REQUIRE(std::is_same_v<decltype(std::declval<Out>().stats), lang::parse_tree_stats>);
}

TEST_CASE("parse_output: default success is false", "[samasa][static_parse]") {
...
TEST_CASE("static_parse_output: third template param MaxDiags controls diagnostics array size", "[samasa][static_parse]") {
...
TEST_CASE("static_parse_output: diag_count is uint32_t", "[samasa][static_parse]") {
    using Out = static_parse_output<SK, TK, 256, 256, 64>;
    STATIC_REQUIRE(std::is_same_v<decltype(std::declval<Out>().diag_count), std::uint32_t>);
}

TEST_CASE("parse_static: capacity-overflow path keeps success=false", "[samasa][static_parse]") {
...
TEST_CASE("parse_static: custom MaxTokens is propagated to return type", "[samasa][static_parse]") {
...
TEST_CASE("S1: parse_static produces events for valid input", "[samasa][static_parse][s1]") {
    constexpr auto out = parse_static<MyGrammar, "hello">(s1_kinds);
    STATIC_REQUIRE(out.success);
    STATIC_REQUIRE(out.event_count > 0);
}

TEST_CASE("S1: parse_static success=false for malformed input — no hard compile error", "[samasa][static_parse][s1]") {
    // Empty source — tok<ident> fails, no hard error.
    constexpr auto out = parse_static<MyGrammar, "">(s1_kinds);
    STATIC_REQUIRE(!out.success);
}

TEST_CASE("S1: parse_static event overflow sets success=false, not a build break", "[samasa][static_parse][s1]") {
    // MaxEvents=0: any event emission overflows → success=false.
    constexpr auto out = parse_static<MyGrammar, "hello", 64, 0, 4>(s1_kinds);
    STATIC_REQUIRE(!out.success);
}

TEST_CASE("S1: parse_static token_count matches scanned tokens", "[samasa][static_parse][s1]") {
    // "hello" scans to [ident, eof] → token_count == 2.
    constexpr auto out = parse_static<MyGrammar, "hello">(s1_kinds);
    STATIC_REQUIRE(out.token_count == 2);
}

TEST_CASE("S1: parse_static consteval/runtime parity — event_count matches", "[samasa][static_parse][s1]") {
