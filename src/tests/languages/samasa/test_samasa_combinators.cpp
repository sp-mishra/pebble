...
TEST_CASE("combinators: seq matches in order", "[samasa][combinators]") {
...
TEST_CASE("combinators: seq rolls back on failure", "[samasa][combinators]") {
...
TEST_CASE("combinators: choice picks first match", "[samasa][combinators]") {
    TestCtx tc{TK::b};
...
TEST_CASE("combinators: choice fails when all alternatives fail", "[samasa][combinators]") {
    TestCtx tc{TK::c};
...
TEST_CASE("combinators: opt succeeds even when inner fails", "[samasa][combinators]") {
    TestCtx tc{TK::c};
...
TEST_CASE("combinators: opt succeeds and consumes when inner matches", "[samasa][combinators]") {
    TestCtx tc{TK::a};
    auto r = opt(tok<TK::a>{}).match(*tc.ctx);
    CHECK(r.ok());
    CHECK(tc.ctx->cursor().pos == 1);
...
TEST_CASE("combinators: many consumes zero tokens on no match", "[samasa][combinators]") {
    TestCtx tc{TK::b};
...
TEST_CASE("combinators: many consumes all matching tokens", "[samasa][combinators]") {
...
TEST_CASE("combinators: sep_by matches list", "[samasa][combinators]") {
...
TEST_CASE("combinators: sep_by succeeds on empty input (zero elements)", "[samasa][combinators]") {
    TestCtx tc{TK::c};
    auto r = sep_by(tok<TK::a>{}, tok<TK::b>{}).match(*tc.ctx);
    CHECK(r.ok());
    CHECK(tc.ctx->cursor().pos == 0);
...
TEST_CASE("combinators: cut upgrades soft_fail to hard_fail in seq", "[samasa][combinators]") {
...
TEST_CASE("combinators: choice does not rewind on hard_fail", "[samasa][combinators]") {
...
TEST_CASE("combinators: backtracking restores event_stream snapshot", "[samasa][combinators]") {
    TestCtx tc{TK::b};
...
TEST_CASE("combinators: choice rolls back diagnostics emitted by failing first branch", "[samasa][combinators]") {
...
TEST_CASE("combinators: choice rolls back repairs emitted by failing first branch", "[samasa][combinators]") {
    TestCtx tc{TK::b};
...
TEST_CASE("combinators: cut inside inner seq causes hard-fail; outer choice does not try next branch", "[samasa][combinators]") {
...
TEST_CASE("combinators: seq without cut still allows choice to try next branch on failure", "[samasa][combinators]") {
...
TEST_CASE("combinators: PEG ordered choice — first match wins, second never tried", "[samasa][combinators]") {
...
TEST_CASE("combinators: PEG ordered choice — second alternative tried only on first soft-fail", "[samasa][combinators]") {
...
TEST_CASE("combinators: lookahead does not consume tokens", "[samasa][combinators]") {
    TestCtx tc{TK::a, TK::b};
...
TEST_CASE("combinators: not_followed_by succeeds when inner fails", "[samasa][combinators]") {
    TestCtx tc{TK::a};
...
TEST_CASE("combinators: not_followed_by fails when inner succeeds", "[samasa][combinators]") {
    TestCtx tc{TK::a};
...
TEST_CASE("combinators: many1 fails on empty match", "[samasa][combinators]") {
    TestCtx tc{TK::b};
...
TEST_CASE("combinators: sep_by1 fails on empty match", "[samasa][combinators]") {
    TestCtx tc{TK::b};
