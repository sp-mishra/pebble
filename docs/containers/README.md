...
| AABBTree            | Policy-driven BVH: 4-way SIMD Highway (`QuadBranchingPolicy`) or 2-way AVL (`BinaryBranchingPolicy`) SAH-lite insertion + refit rebalance | `spatial/AABBTree.hpp` & `tree/AABBTree.hpp` |
| SparseSet           | Dual-buffer sparse set, O(1) insert/erase/contains; unified `FlatSparsePolicy` & `PagedSparsePolicy` for multi-million keys | `associative/SparseSet.hpp`      |
| slot_map            | Generational slot map, stable-address O(1) insert/erase/lookup keyed by `generational_handle`, bulk `reserve` | `associative/slot_map.hpp`       |
...
| static_vector       | Fixed-capacity uninitialized byte-aligned storage, never allocates, constexpr-compatible                                                                                                          | `static/static_vector.hpp`       |
...
| descriptor_registry | Stable generational handles via slot_map; SparseSet ID index + FNV-1a name hashing                                                                                                                | `descriptor_registry.hpp`        |
